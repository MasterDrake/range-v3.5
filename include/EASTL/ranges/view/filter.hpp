...
namespace ranges
{
    ...
    {
        filter_view() = default;
        constexpr filter_view(Rng rng, Pred pred) : filter_view::remove_if_view{eastl::move(rng), not_fn(eastl::move(pred))}
        {}
    };
    ...
    template(typename Rng, typename Pred)(
        requires input_range<Rng> AND indirect_unary_predicate<Pred, iterator_t<Rng>> AND
            view_<Rng> AND eastl::is_object<Pred>::value) //
    ...
    namespace views
    {
        ...
        struct cpp20_filter_base_fn
        {
            ...
            constexpr filter_view<all_t<Rng>, Pred> operator()(Rng && rng, Pred pred) //
                const
            {
                return filter_view<all_t<Rng>, Pred>{all(static_cast<Rng &&>(rng)), eastl::move(pred)};
            }
        };
        ...
        struct cpp20_filter_fn : cpp20_filter_base_fn
        {
            ...
            {
                return make_view_closure(bind_back(cpp20_filter_base_fn{}, eastl::move(pred)));
            }
        };
        ...
        struct filter_base_fn : cpp20_filter_base_fn
        {
            ...
            template(typename Rng, typename Pred, typename Proj)(requires viewable_range<Rng> AND input_range<Rng> AND indirect_unary_predicate<Pred, projected<iterator_t<Rng>, Proj>>)
            constexpr filter_view<all_t<Rng>, composed<Pred, Proj>> //
            operator()(Rng && rng, Pred pred, Proj proj) const
            {
                return filter_view<all_t<Rng>, composed<Pred, Proj>>{all(static_cast<Rng &&>(rng)), compose(eastl::move(pred), eastl::move(proj))};
            }
        };
        ...
        struct filter_fn : filter_base_fn
        {
            ...
            constexpr auto operator()(Pred pred) const
            {
                return make_view_closure(bind_back(filter_base_fn{}, eastl::move(pred)));
            }

            template(typename Pred, typename Proj)(requires (!range<Pred>))
            constexpr auto operator()(Pred pred, Proj proj) const
            {
                return make_view_closure(bind_back(filter_base_fn{}, eastl::move(pred), eastl::move(proj)));
            }
        };
