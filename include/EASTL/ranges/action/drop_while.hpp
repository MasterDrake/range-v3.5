...
namespace ranges
{
    ...
    namespace actions
    {
        struct drop_while_fn
        {
            ...
            {
                return make_action_closure(bind_back(drop_while_fn{}, eastl::move(fun)));
            }

            template(typename Rng, typename Fun)(requires forward_range<Rng> AND indirect_unary_predicate<Fun, iterator_t<Rng>> AND erasable_range<Rng &, iterator_t<Rng>, iterator_t<Rng>>)
            Rng operator()(Rng && rng, Fun fun) const
            {
                ranges::actions::erase(rng, begin(rng), find_if_not(begin(rng), end(rng), eastl::move(fun)));
