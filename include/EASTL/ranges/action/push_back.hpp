...
#include "../action/insert.hpp"
#include "../detail/with_braced_init_args.hpp"
...
namespace ranges
{
    ...
    namespace adl_push_back_detail
    {
        ...
        using push_back_t = decltype(static_cast<void>(unwrap_reference(eastl::declval<Cont &>()).push_back(eastl::declval<T>())));

        template<typename Cont, typename Rng>
        using insert_t = decltype(static_cast<void>(ranges::insert(eastl::declval<Cont &>(), eastl::declval<sentinel_t<Cont>>(), eastl::declval<Rng>())));
        ...
        struct push_back_fn
        {
            ...
            {
                return make_action_closure(bind_back(push_back_fn{}, static_cast<T &&>(val)));
            }
            ...
            {
                return make_action_closure(bind_back(push_back_fn{}, detail::reference_wrapper_<T>(t)));
            }
            ...
            template(typename Rng, typename T)(requires input_range<Rng> AND can_push_back_<Rng, T> AND (range<T> || constructible_from<range_value_t<Rng>, T>)) //
