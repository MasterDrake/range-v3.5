...
namespace ranges
{
    ...
    namespace _end_
    {
        ...
        public:
            ...
            template(typename R)(requires detail::_borrowed_range<R> AND has_member_end<R>)
            constexpr _result_t<R> operator()(R && r) const //
            ...
            template(typename R)(requires detail::_borrowed_range<R> AND (!has_member_end<R>) AND has_non_member_end<R>)
            constexpr _result_t<R> operator()(R && r) const //
            ...
            auto operator-(Int dist) const -> detail::from_end_<iter_diff_t<Int>>
            {
                using SInt = iter_diff_t<Int>;
                EARANGES_EXPECT(0 <= dist);
                EARANGES_EXPECT(dist <=  static_cast<Int>((eastl::numeric_limits<SInt>::max)()));
