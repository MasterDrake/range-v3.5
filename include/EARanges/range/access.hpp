...
#ifndef EARANGES_RANGE_ACCESS_HPP
#define EARANGES_RANGE_ACCESS_HPP
...
namespace eastl
{
    namespace ranges
    {
        ...
        EARANGES_INLINE_VAR constexpr bool
            enable_borrowed_range<eastl::basic_string_view<CharT>> = true;

        template<class T, std::size_t N>
        EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<eastl::span<T, N>> =
            true;
        ...
        namespace _begin_
        {
            ...
            template(typename I)(requires input_or_output_iterator<I>) void is_iterator(
                I);
        ...
        CPP_concept has_non_member_begin =
            ...
            public:
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND
                                         has_member_begin<R>) constexpr _result_t<R>
                operator()(R && r) const //
                ...
                template(typename R)(
                    requires detail::_borrowed_range<R> AND(!has_member_begin<R>)
                        AND has_non_member_begin<R>) constexpr _result_t<R>
                operator()(R && r) const //
        ...
        /// is
        ///   well-formed and returns an input_or_output_iterator. Otherwise, `begin(r)`
        ///   if that expression returns an input_or_output_iterator.
        EARANGES_DEFINE_CPO(_begin_::fn, begin)
        ...
        namespace _end_
        {
            ...
            template(typename I,
                     typename S)(requires sentinel_for<S, I>) void _is_sentinel(S, I);
        ...
        CPP_concept has_non_member_end =
            ...
            private:
                ...
                using iter_diff_t =
                    meta::_t<meta::conditional_t<eastl::is_integral<Int>::value,
                                                 eastl::make_signed<Int>, //
                                                 meta::id<Int>>>;
            ...
            public:
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND
                                         has_member_end<R>) constexpr _result_t<R>
                operator()(R && r) const //
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND(
                    !has_member_end<R>) AND has_non_member_end<R>) constexpr _result_t<R>
                operator()(R && r) const //
                ...
                template(typename Int)(requires detail::integer_like_<Int>) auto
                operator-(Int dist) const -> detail::from_end_<iter_diff_t<Int>>
                {
                    using SInt = iter_diff_t<Int>;
                    EARANGES_EXPECT(0 <= dist);
                    EARANGES_EXPECT(
                        dist <= static_cast<Int>((eastl::numeric_limits<SInt>::max)()));
        ...
        /// expression is
        ///   well-formed and returns an input_or_output_iterator. Otherwise, `end(r)` if
        ///   that expression returns an input_or_output_iterator.
        EARANGES_DEFINE_CPO(_end_::fn, end)
        ...
        ///    reference to r.
        EARANGES_INLINE_VARIABLE(_cbegin_::fn, cbegin)
        ...
        ///    reference to r.
        EARANGES_INLINE_VARIABLE(_cend_::fn, cend)
        ...
        CPP_concept can_reverse_end =
            ...
            private:
                ...
                {
                    template<typename R>
                    using invoke = decltype(ranges::make_reverse_iterator(
                        ranges::end(declval(R &))));
                };
                struct _other_result_
                {
                    template<typename R>
                    using invoke = meta::invoke<
                        meta::conditional_t<has_non_member_rbegin<R>, _non_member_result_,
                                            _reverse_result_>,
                        R>;
                };
                ...
                template<typename R>
                using _result_t =
                    meta::invoke<meta::conditional_t<has_member_rbegin<R>,
                                                     _member_result_, _other_result_>,
                                 R>;

            public:
                template(typename R)(requires detail::_borrowed_range<R> AND
                                         has_member_rbegin<R>) constexpr auto
                operator()(R && r) const           //
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND(
                    !has_member_rbegin<R>) AND has_non_member_rbegin<R>) constexpr auto
                operator()(R && r) const          //
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND(
                    !has_member_rbegin<R>) AND(!has_non_member_rbegin<R>)
                                         AND can_reverse_end<R>) constexpr auto
                operator()(R && r) const //
        ...
        /// \ingroup group-range
        /// \param r
        /// \return `make_reverse_iterator(r + ranges::size(r))` if r is an array.
        /// Otherwise,
        ///   `r.rbegin()` if that expression is well-formed and returns an
        ///   input_or_output_iterator. Otherwise, `make_reverse_iterator(ranges::end(r))`
        ///   if `ranges::begin(r)` and `ranges::end(r)` are both well-formed and have the
        ///   same type that satisfies `bidirectional_iterator`.
        EARANGES_DEFINE_CPO(_rbegin_::fn, rbegin)
        ...
        CPP_concept can_reverse_begin =
            ...
            private:
                ...
                struct _reverse_result_
                {
                    template<typename R>
                    using invoke = decltype(ranges::make_reverse_iterator(
                        ranges::begin(declval(R &))));
                };
                struct _other_result_
                {
                    template<typename R>
                    using invoke = meta::invoke<
                        meta::conditional_t<has_non_member_rend<R>, _non_member_result_,
                                            _reverse_result_>,
                        R>;
                };
                ...
                template<typename R>
                using _result_t =
                    meta::invoke<meta::conditional_t<has_member_rend<R>, _member_result_,
                                                     _other_result_>,
                                 R>;

            public:
                template(typename R)(requires detail::_borrowed_range<R> AND
                                         has_member_rend<R>) constexpr auto
                operator()(R && r) const         //
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND(
                    !has_member_rend<R>) AND has_non_member_rend<R>) constexpr auto
                operator()(R && r) const        //
                ...
                template(typename R)(requires detail::_borrowed_range<R> AND(
                    !has_member_rend<R>) AND(!has_non_member_rend<R>)
                                         AND can_reverse_begin<R>) constexpr auto
                operator()(R && r) const //
        ...
        EARANGES_DEFINE_CPO(_rend_::fn, rend)
        ...
        ///    reference to r.
        EARANGES_INLINE_VARIABLE(_crbegin_::fn, crbegin)
        ...
        ///    reference to r.
        EARANGES_INLINE_VARIABLE(_crend_::fn, crend)
