...
#ifndef EARANGES_RANGE_CONVERSION_HPP
#define EARANGES_RANGE_CONVERSION_HPP
...
namespace eastl
{
    namespace ranges
    {
        ...
        using container_t = meta::invoke<MetaFn, Rng>;
            ...
            friend constexpr auto operator|(to_container::closure<MetaFn, Fn> sh, Pipeable pipe)
                -> CPP_broken_friend_ret(
        ...
        public:
            template(typename Rng)(requires input_range<Rng> AND convertible_to_cont<Rng, container_t<MetaFn, Rng>>)
            container_t<MetaFn, Rng> operator()(Rng && rng) const
            {
                static_assert(!is_infinite<Rng>::value, "Attempt to convert an infinite range to a container.");
                using cont_t = container_t<MetaFn, Rng>;
                using iter_t = range_cpp17_iterator_t<Rng>;
                using use_reserve_t = meta::bool_<(bool)to_container_reserve<cont_t, iter_t, Rng>>;
                return impl<cont_t, iter_t>(static_cast<Rng &&>(rng), use_reserve_t{});
            }
            template(typename Rng)(requires input_range<Rng> AND (!convertible_to_cont<Rng, container_t<MetaFn, Rng>>) AND convertible_to_cont_cont<Rng, container_t<MetaFn, Rng>>)
            container_t<MetaFn, Rng> operator()(Rng && rng) const
            {
                static_assert(!is_infinite<Rng>::value, "Attempt to convert an infinite range to a container.");
                using cont_t = container_t<MetaFn, Rng>;
    ...
    /// \ingroup group-range
    EARANGES_INLINE_VARIABLE(detail::to_container_fn<detail::from_range<eastl::vector>>, to_vector)
    ...
    namespace _to_
    {
        ...
        /// an Range
        template<template<typename...> class ContT>
        auto to(EARANGES_HIDDEN_DETAIL(detail::to_container = {}))
        ...
        template(template<typename...> class ContT, typename Rng, typename C = meta::invoke<detail::from_range<ContT>, Rng>)(
            requires range<Rng> AND detail::convertible_to_cont<Rng, C>)
        auto to(Rng && rng) -> C
        {
        ...
        auto to(EARANGES_HIDDEN_DETAIL(detail::to_container = {})) -> detail::to_container_fn<meta::id<Cont>>
        {
            return {};
        }

        /// \overload
        template(typename Cont, typename Rng)(requires range<Rng> AND detail::convertible_to_cont<Rng, Cont>)
        ...
        template(template<typename...> class ContT, typename T)(requires detail::convertible_to_cont<std::initializer_list<T>, ContT<T>>)
        auto to(std::initializer_list<T> il) -> ContT<T>
        {
            return detail::to_container_fn<detail::from_range<ContT>>{}(il);
        }
        template(typename Cont, typename T)(requires detail::convertible_to_cont<std::initializer_list<T>, Cont>)
