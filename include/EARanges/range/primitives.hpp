    ...
    namespace ranges
    {
        ...
        CPP_concept has_non_member_size =
            ...
            private:
                ...
                struct _other_result_
                {
                    ...
                    using invoke = meta::invoke<
                        meta::conditional_t<has_non_member_size<R>, _non_member_result_,
                                            _distance_result_>,
                        R>;
                };
                ...
                template<typename R>
                using _result_t =
                    meta::invoke<meta::conditional_t<has_member_size<R>, _member_result_,
                                                     _other_result_>,
                                 R>;
            ...
            public:
                ...
                template(typename R)(
                    requires(!disable_sized_range<uncvref_t<R>>) AND has_member_size<R>
                        AND detail::integer_like_<_result_t<R>>) constexpr _result_t<R>
                operator()(R && r) const noexcept(noexcept(((R &&)r).size()))
                {
                    return ((R &&)r).size();
                }
                ...
                template(typename R)(
                    requires(!disable_sized_range<uncvref_t<R>>) AND(
                        !has_member_size<R>) AND has_non_member_size<R>
                        AND detail::integer_like_<_result_t<R>>) constexpr _result_t<R>
                operator()(R && r) const noexcept(noexcept(size((R &&)r)))
                {
                    return size((R &&)r);
                }

                template(typename R)(
                    requires(!has_member_size<R> || disable_sized_range<uncvref_t<R>>)
                        AND(!has_non_member_size<R> || disable_sized_range<uncvref_t<R>>)
                            AND forward_iterator<_begin_::_t<R>>
                                AND sized_sentinel_for<
                                    _end_::_t<R>, _begin_::_t<R>>) constexpr _result_t<R>
                operator()(R && r) const
        ...
        /// \ingroup group-range
        /// \return For a given expression `E` of type `T`, `ranges::size(E)` is
        /// equivalent to:
        ...
        ///     the declaration: \code template<class T> void size(T&&) = delete; \endcode
        ///     and does not include a declaration of `ranges::size`, and
        ///     `disable_sized_range<eastl::remove_cvref_t<T>>` is false.
        ///   * Otherwise, `static_cast<U>(ranges::end(E) - ranges::begin(E))` where `U`
        ///   is
        ...
        ///     `ranges::begin(E)` and `ranges::end(E)` model `sized_sentinel_for<S, I>`
        ...
        CPP_concept has_member_data =
            ...
            private:
                ...
                struct _other_result_
                {
                    template<typename R>
                    using invoke = meta::invoke<
                        meta::conditional_t<eastl::is_pointer<_begin_::_t<R>>::value,
                                            _pointer_iterator_, _contiguous_iterator_>,
                        R>;
                };

                template<typename R>
                using _result_t =
                    meta::invoke<meta::conditional_t<has_member_data<R>, _member_data_,
                                                     _other_result_>,
                                 R>;
            ...
            public:
                template(typename R)(
                    requires has_member_data<R &> AND
                        eastl::is_pointer<_result_t<R &>>::value) constexpr _result_t<R &>
                operator()(R & r) const //
                ...
                template(typename R)(
                    requires(!has_member_data<R &>) AND
                        eastl::is_pointer<_begin_::_t<R>>::value) constexpr _result_t<R>
                operator()(R && r) const //
                ...
                template(typename R)(
                    requires(!has_member_data<R &>) AND(
                        ...
                        AND contiguous_iterator<_begin_::_t<R>>) constexpr _result_t<R>
                operator()(R && r) const //
                ...
                constexpr charT * operator()(
                    eastl::basic_string<charT, Alloc> & s) const noexcept
                {
        ...
        CPP_concept has_size =
            ...
            struct fn
            {
                ...
                template(typename R)(requires has_member_empty<R>) constexpr bool
                operator()(R && r) const noexcept(noexcept(bool(((R &&)r).empty())))
                {
                ...
                template(typename R)(requires(!has_member_empty<R>) AND(
                    !has_size<R>) AND forward_iterator<_begin_::_t<R>>) constexpr bool
                operator()(R && r) const noexcept(noexcept(bool(ranges::begin((R &&)r) ==
                                                                ranges::end((R &&)r))))
                {
