...
#ifndef EARANGES_DETAIL_VARIANT_HPP
#define EARANGES_DETAIL_VARIANT_HPP

#include <EASTL/iterator.h>
//TODO: Do domething about these new calls :O
#include <new>
#include <stdexcept>
    ...
    namespace ranges
{
    ...
    namespace detail
    {
        ...
        O uninitialized_copy(I first, S last, O out)
        {
            for(; first != last; ++first, ++out)
                ::new((void *)eastl::addressof(*out)) iter_value_t<O>(*first);
            return out;
        }
        ...
        O uninitialized_copy(I first, S last, O out)
        {
            return eastl::uninitialized_copy_n(first, (last - first), out);
        }

        template<typename I, typename O>
        O uninitialized_copy(I first, I last, O out)
        {
            return eastl::uninitialized_copy(first, last, out);
        }
        ...
        public:
            ...
            constexpr indexed_datum(Ts &&... ts) noexcept(
                    eastl::is_nothrow_constructible<T, Ts...>::value)
        ...
        template<typename Ts,
                 bool Trivial = meta::apply<
                     ...
                     meta::transform<Ts, meta::quote<eastl::is_trivially_destructible>>>::
                     type::value>
        ...
        struct construct_fn
        {
            ...
            {
                ::new((void *)eastl::addressof(u))
                    U(static_cast<Ts &&>(eastl::get<Is>(args_))...);
            }
            ...
            template<typename U>
            meta::if_<eastl::is_object<U>> operator()(
                indexed_datum<U, meta::size_t<N>> &
                    u) noexcept(eastl::is_nothrow_constructible<U, Ts...>::value)
            {
                this->construct_(u.get(), meta::make_index_sequence<sizeof...(Ts)>{});
            }
            template<typename U>
            meta::if_<meta::not_<eastl::is_object<U>>> operator()(
                indexed_datum<U, meta::size_t<N>> &
                    u) noexcept(eastl::is_nothrow_constructible<detail::decay_t<U>,
                                                              Ts...>::value)
            {
        ...
        struct get_fn
        {
            ...
            {
                *t_ = eastl::addressof(t.get());
            }
            template<typename U>
            void operator()(indexed_element<U &&, N> t) const noexcept
            {
                U && u = t.get();
                *t_ = eastl::addressof(u);
            }
        ...
        variant_visitor<Fun, Variant> make_variant_visitor(
            Variant & var,
            Fun fun) noexcept(eastl::is_nothrow_move_constructible<Fun>::value)
        {
        ...
        struct variant_get
        {
            ...
            get(Variant & var)
            {
                using elem_t = meta::_t<eastl::remove_reference<meta::at_c<meta::as_list<Variant>, N>>>;
                elem_t * elem = nullptr;
            ...
            {
                using elem_t = meta::_t<eastl::remove_reference<meta::at_c<meta::as_list<Variant>, N> const>>;
                elem_t * elem = nullptr;
                auto & data_var = detail::variant_core_access::data(var);
                detail::variant_visit_(var.index(), data_var, detail::get_fn<elem_t, N>{&elem});
                return detail::variant_deref_(elem);
            ...
            get(Variant && var)
            {
                ...
                auto & data_var = detail::variant_core_access::data(var);
                detail::variant_visit_(var.index(), data_var, detail::get_fn<elem_t, N>{&elem});
                using res_t = meta::_t<eastl::add_rvalue_reference<meta::at_c<meta::as_list<Variant>, N>>>;
                return static_cast<res_t>(detail::variant_deref_(elem));
            }
        };

        template<typename Variant,
                 bool Trivial = eastl::is_trivially_destructible<meta::apply<
    ...
    public:
        ...
        constexpr CPP_ctor(variant)()(                                         //
            noexcept(eastl::is_nothrow_default_constructible<datum_t<0>>::value) //
        ...
        void emplace(Args &&... args)
        {
            ...
            detail::variant_visit_(N, data_(), eastl::ref(fn), identity{});
            index_ = N;
        }
