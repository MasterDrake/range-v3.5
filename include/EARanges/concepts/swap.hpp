    ...
    namespace concepts
    {
        ...
        namespace detail
        {
            ...
            CPP_INLINE_VAR constexpr bool is_movable_v =
                eastl::is_object<T>::value && eastl::is_move_constructible<T>::value &&
        ...
        CPP_CXX14_CONSTEXPR meta::if_c<eastl::is_move_constructible<T>::value &&
                                           eastl::is_assignable<T &, U>::value,
                                       T>
        exchange(T & t, U && u) noexcept(eastl::is_nothrow_move_constructible<T>::value &&
        ...
        {
            // Intentionally create an ambiguity with eastl::swap, which is (possibly)
            // unconstrained.
            template<typename T>
            ...
            template<typename T, typename U = T>
            CPP_INLINE_VAR constexpr bool is_adl_swappable_v = !META_IS_SAME(
                decltype(adl_swap_detail::try_adl_swap_<T, U>(42)), eastl::unused);

            struct swap_fn
            {
                // Dispatch to user-defined swap found via ADL:
                template<typename T, typename U>
                CPP_CXX14_CONSTEXPR meta::if_c<is_adl_swappable_v<T, U>> operator()(
                    T && t, U && u) const noexcept(noexcept(swap((T &&)t, (U &&)u)))
                {
                    swap((T &&)t, (U &&)u);
                }
                ...
                template<typename T>
                CPP_CXX14_CONSTEXPR meta::if_c<!is_adl_swappable_v<T &> &&
                ...
                CPP_CXX14_CONSTEXPR meta::if_c<!is_adl_swappable_v<T (&)[N], U (&)[N]> &&
                ...
                CPP_CXX14_CONSTEXPR meta::if_c<is_swappable_with<F0, F1>::value &&
                                               is_swappable_with<S0, S1>::value>
                operator()(eastl::pair<F0, S0> && left,
                           eastl::pair<F1, S1> && right) const
                ...
                CPP_CXX14_CONSTEXPR meta::if_c<
                    meta::and_c<is_swappable_with<Ts, Us>::value...>::value>
                operator()(eastl::tuple<Ts...> && left,
                           eastl::tuple<Us...> && right) const
                    noexcept(
                        meta::and_c<is_nothrow_swappable_with<Ts, Us>::value...>::value)
                {
            ...
            private:
                ...
                CPP_CXX14_CONSTEXPR static void impl(T && left, U && right,
            ...
            struct is_swappable_with_ : eastl::false_type
            {};

            template<typename T, typename U>
            struct is_swappable_with_<
                T, U,
                meta::void_<decltype(swap_fn()(eastl::declval<T>(), eastl::declval<U>())),
                            decltype(swap_fn()(eastl::declval<U>(),
                                               eastl::declval<T>()))>> : eastl::true_type
            {};

            template<typename T, typename U>
            struct is_nothrow_swappable_with_
              : meta::bool_<noexcept(
                                swap_fn()(eastl::declval<T>(), eastl::declval<U>())) &&
                            noexcept(swap_fn()(eastl::declval<U>(), eastl::declval<T>()))>
            {};

            // Q: Should eastl::reference_wrapper be considered a proxy wrt swapping
            // rvalues? A: No. Its operator= is currently defined to reseat the
            // references, so
            ...
            //    referents unmodified. Treating rvalue reference_wrappers differently
            //    would be confusing.

            // Q: Then why is it OK to "re"-define swap for pairs and tuples of
            // references? A: Because as defined above, swapping an rvalue tuple of
            // references has the same
            //    semantics as swapping an lvalue tuple of references. Rather than reseat
            //    the references, assignment happens *through* the references.

            // Q: But I have an iterator whose operator* returns an rvalue
            //    eastl::reference_wrapper<T>. How do I make it model
