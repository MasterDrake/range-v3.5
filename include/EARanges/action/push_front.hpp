...
#ifndef EARANGES_ACTION_PUSH_FRONT_HPP
#define EARANGES_ACTION_PUSH_FRONT_HPP
...
namespace ranges
{
    ...
    namespace adl_push_front_detail
    {
        ...
        using push_front_t = decltype(static_cast<void>(unwrap_reference(eastl::declval<Cont &>()).push_front(eastl::declval<T>())));

        template<typename Cont, typename Rng>
        using insert_t = decltype(static_cast<void>(ranges::insert(eastl::declval<Cont &>(), eastl::declval<iterator_t<Cont>>(), eastl::declval<Rng>())));

        template(typename Cont, typename T)(requires lvalue_container_like<Cont> AND (!range<T>) AND constructible_from<range_value_t<Cont>, T>)
        ...
        struct push_front_fn
        {
            ...
            {
                return make_action_closure(bind_back(push_front_fn{}, detail::reference_wrapper_<T>(t)));
            }

            template(typename Rng, typename T)(requires input_range<Rng> AND can_push_front_<Rng, T> AND (range<T> || constructible_from<range_value_t<Rng>, T>)) //
            ...
            template(typename Rng, typename T)(requires input_range<Rng> AND can_push_front_<Rng, std::initializer_list<T>> AND constructible_from<range_value_t<Rng>, T const &>)
    ...
    namespace actions
    {
        EARANGES_INLINE_VARIABLE(adl_push_front_detail::push_front_fn, push_front)
    } // namespace actions
