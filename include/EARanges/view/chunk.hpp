    ...
    namespace ranges
    {
        ...
        private:
            ...
            public:
                        ...
                        requires(detail::can_sized_sentinel_<Rng, Const>()))
                {
                    ...
                    // divide the cycle length. Such iterator pairs are NOT in the domain
                    // of
                    // -.
        ...
        private:
            ...
            private:
                ...
                private:
                    ...
                    constexpr void next()
                    {
                        ...
                        ++rng_->it();
                        --rng_->remainder_;
                        if(rng_->remainder_ != 0 &&
                           rng_->it() == ranges::end(rng_->base_))
                            rng_->remainder_ = 0;
                    }
                    CPP_member
            ...
            public:
                ...
                {
                    EARANGES_EXPECT(rng_);
                    return rng_->it() == ranges::end(rng_->base_) &&
                           rng_->remainder_ != 0;
                }
                ...
                constexpr void next()
                {
                    EARANGES_EXPECT(!done());
                    ranges::advance(
                        rng_->it(), rng_->remainder_, ranges::end(rng_->base_));
        ...
        template<typename Rng>
        EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<chunk_view<Rng>> =
            ...
            struct chunk_base_fn
            {
                template(typename Rng)(
                    requires viewable_range<Rng> AND
                        input_range<Rng>) constexpr chunk_view<all_t<Rng>> //
