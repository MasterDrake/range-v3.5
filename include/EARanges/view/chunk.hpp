...
#ifndef EARANGES_VIEW_CHUNK_HPP
#define EARANGES_VIEW_CHUNK_HPP
...
#include <EARanges/utility/optional.hpp> // for non_propagating_cache
...
namespace ranges
{
    ...
    private:
        ...
        private:
            ...
            constexpr offset_t<Const> const & offset() const
            {
                ...
                EARANGES_EXPECT(0 <= result && result < n_);
                return result;
            }
        ...
        public:
            ...
            constexpr adaptor(meta::const_if_c<Const, chunk_view_> * cv)
              ...
              , n_((EARANGES_EXPECT(0 < cv->n_), cv->n_))
            ...
            {
                EARANGES_EXPECT(it != end_);
                EARANGES_EXPECT(0 == offset());
                    ...
                    requires (detail::can_sized_sentinel_<Rng, Const>()))
            {
                ...
                // -.
                EARANGES_ENSURE(0 == delta % n_);
                return delta / n_;
            }
            CPP_member
            constexpr auto advance(iterator_t<CRng> & it, range_difference_t<Rng> n) //
                -> CPP_ret(void)(requires random_access_range<CRng>)
            {
                using Limits = eastl::numeric_limits<range_difference_t<CRng>>;
                if(0 < n)
                {
                    EARANGES_EXPECT(0 == offset());
                    EARANGES_EXPECT(n <= Limits::max() / n_);
                    auto const remainder = ranges::advance(it, n * n_, end_) % n_;
                    EARANGES_EXPECT(0 <= remainder && remainder < n_);
                    offset() = remainder;
                }
                else if(0 > n)
                {
                    EARANGES_EXPECT(n >= Limits::min() / n_);
        ...
        constexpr auto begin_adaptor() const //
            -> CPP_ret(adaptor<true>)(requires forward_range<Rng const>)
        {
        ...
        private:
            ...
            private:
                ...
                constexpr auto distance_to(default_sentinel_t) const
                    -> CPP_ret(range_difference_t<Rng>)(requires sized_sentinel_for<sentinel_t<Rng>, iterator_t<Rng>>)
                {
                    EARANGES_EXPECT(rng_);
            ...
            public:
                ...
                {}
                CPP_auto_member
                constexpr auto CPP_fun(size)()(requires sized_sentinel_for<sentinel_t<Rng>, iterator_t<Rng>>)
                {
    ...
    {
        chunk_view() = default;
        constexpr chunk_view(Rng rng, range_difference_t<Rng> n) : chunk_view_<Rng, (bool)forward_range<Rng>>(static_cast<Rng &&>(rng), n)
        {}
    };

    // Need to keep extra state for input_range, but forward_range is transparent
    template<typename Rng>
    EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<chunk_view<Rng>> = enable_borrowed_range<Rng> && forward_range<Rng>;
    ...
    namespace views
    {
        ...
        /// \relates chunk_fn
        /// \ingroup group-views
        EARANGES_INLINE_VARIABLE(chunk_fn, chunk)
    } // namespace views
    /// @}
} // namespace ranges
...
EARANGES_SATISFY_BOOST_RANGE(::ranges::chunk_view)
