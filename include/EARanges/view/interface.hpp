...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            struct slice_bounds
            {
                ...
                template(typename F, typename T)(
                    requires convertible_to<F, From> AND
                        convertible_to<T, To>) constexpr slice_bounds(F f, T t)
            ...
            struct from_end_
            {
                ...
                template(typename Other)(
                    requires integer_like_<Other> AND
                        explicitly_convertible_to<Other, Int>) constexpr
        ...
        CPP_concept can_empty_ = //
            ...
            EARANGES_INLINE_VAR constexpr bool has_fixed_size_ =
                (C >= 0 || C == infinite);
        ...
        public:
            ...
            template(bool True = true)(
                requires True AND(Cardinality < 0) AND(Cardinality != infinite)
                    AND(!forward_range<D<True>>)
                        AND sized_range<D<True>>) constexpr bool empty() //
            ...
            template(bool True = true)(
                requires True AND(Cardinality < 0) AND(Cardinality != infinite)
                    AND(!forward_range<D<True> const>)
                        AND sized_range<D<True> const>) constexpr bool empty() const //
                noexcept(noexcept(bool(ranges::size(eastl::declval<D<True> const &>()) ==
                                       0)))
            {
                return ranges::size(derived()) == 0;
            }
            /// \overload
            template(bool True = true)(
                requires True AND(!detail::has_fixed_size_<Cardinality>) AND forward_range<D<
                    True>>) constexpr bool empty() noexcept(noexcept(bool(ranges::
                                                                              begin(eastl::declval<
                                                                                    D<True> &>()) ==
                                                                          ranges::end(
                                                                              eastl::declval<D<
            ...
            /// \overload
            template(bool True = true)(
                requires True AND(!detail::has_fixed_size_<Cardinality>)
                    AND forward_range<D<True> const>) constexpr bool empty() const
                noexcept(noexcept(bool(ranges::begin(eastl::declval<D<True> const &>()) ==
                                       ranges::end(eastl::declval<D<True> const &>()))))
            {
                return bool(ranges::begin(derived()) == ranges::end(derived()));
            }
            ...
            noexcept(noexcept(ranges::empty(eastl::declval<D<True> const &>())))
        {
            ...
            /// return it.
            template(bool True = true, int = 42)(requires True AND(Cardinality >= 0)) //
            ...
            template(bool True = true)(
                requires True AND(Cardinality < 0)
                    AND sized_sentinel_for<sentinel_t<D<True>>, iterator_t<D<True>>>
                        AND forward_range<D<True>>) constexpr detail::
                iter_size_t<iterator_t<D<True>>> size()
            {
            ...
            template(bool True = true)(
                requires True AND(Cardinality < 0)
                    AND sized_sentinel_for<sentinel_t<D<True> const>,
                                           iterator_t<D<True> const>>
                        AND forward_range<D<True> const>) constexpr detail::
                iter_size_t<iterator_t<D<True>>> size() const //
            {
                using size_type = detail::iter_size_t<iterator_t<D<True>>>;
                return static_cast<size_type>(derived().end() - derived().begin());
            }
            ...
            template(bool True = true)(
                requires True AND
                    forward_range<D<True>>) constexpr range_reference_t<D<True>> front()
            {
                return *derived().begin();
            }
            /// \overload
            template(bool True = true)(
                requires True AND forward_range<
                    D<True> const>) constexpr range_reference_t<D<True> const> front()
                const
            {
                return *derived().begin();
            }
            ...
            template(bool True = true)(
                requires True AND common_range<D<True>> AND bidirectional_range<
                    D<True>>) constexpr range_reference_t<D<True>> back()
            {
                return *prev(derived().end());
            }
            /// \overload
            template(bool True = true)(
                requires True AND common_range<D<True> const> AND bidirectional_range<
                    D<True> const>) constexpr range_reference_t<D<True> const> back()
                const
            {
                return *prev(derived().end());
            }
            ...
            template(bool True = true)(
                requires True AND
                    random_access_range<D<True>>) constexpr range_reference_t<D<True>>
            operator[](range_difference_t<D<True>> n)
            {
                return derived().begin()[n];
            }
            /// \overload
            template(bool True = true)(
                requires True AND random_access_range<
                    D<True> const>) constexpr range_reference_t<D<True> const> //
            ...
            template(bool True = true)(
                requires True AND
                    contiguous_iterator<iterator_t<D<True>>>) constexpr eastl::
                add_pointer_t<range_reference_t<D<True>>> data() //
            {
            ...
            template(bool True = true)(
                requires True AND
                    contiguous_iterator<iterator_t<D<True> const>>) constexpr eastl::
                add_pointer_t<range_reference_t<D<True> const>> data() const //
            {
                return eastl::addressof(*ranges::begin(derived()));
            }
            ...
            /// checking.
            template(bool True = true)(
                requires True AND random_access_range<D<True>> AND sized_range<D<
                    True>>) constexpr range_reference_t<D<True>> at(range_difference_t<D<True>>
                                                                        n)
            {
            ...
            /// \overload
            template(bool True = true)(
                requires True AND random_access_range<D<True> const> AND sized_range<D<
                    True> const>) constexpr range_reference_t<D<True> const> at(range_difference_t<D<True>>
                                                                                    n)
                const
            {
            ...
            template(bool True = true, typename Slice = views::slice_fn)(
                requires True AND input_range<D<True> &>) constexpr auto
            ...
            /// \overload
            template(bool True = true, typename Slice = views::slice_fn)(
                requires True AND input_range<D<True> const &>) constexpr auto
            ...
            /// \overload
            template(bool True = true, typename Slice = views::slice_fn)(
                requires True AND input_range<D<True> &> AND
                    sized_range<D<True> &>) constexpr auto //
            ...
            template(bool True = true, typename Slice = views::slice_fn)(
                requires True AND input_range<D<True> const &> AND
                    sized_range<D<True> const &>) constexpr auto //
            operator[](detail::slice_bounds<range_difference_t<D<True>>,
                                            detail::from_end_of_t<D<True>>>
                           offs) const &
            {
            ...
            /// \overload
            template(bool True = true,
                     typename Slice = views::slice_fn)(requires True AND(
            ...
            template(bool True = true,
                ...
                (input_range<D<True> const &> && sized_range<D<True> const &>))) //
                constexpr auto                                                   //
                operator[](detail::slice_bounds<detail::from_end_of_t<D<True>>, end_fn>
                               offs) const &
            {
                return Slice{}(derived(), offs.from, offs.to);
            }
