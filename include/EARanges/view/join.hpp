...
#ifndef EARANGES_VIEW_JOIN_HPP
#define EARANGES_VIEW_JOIN_HPP
...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
                ...
                return Outer == infinite || Inner == infinite ||
                       ...
                       : Outer == finite || Inner == finite ||
                           ...
                           : static_cast<cardinality>(
                                 Outer * Inner + (Outer == 0 ? 0 : (Outer - 1) * Joiner));
            }
            ...
            struct store_inner_
            {
                non_propagating_cache<eastl::remove_cv_t<Inner>> inner_ = {};
            ...
            using join_view_inner =
                meta::conditional_t<!eastl::is_reference<range_reference_t<Rng>>::value,
        ...
        CPP_concept has_arrow_ =
            input_iterator<I> &&
            (eastl::is_pointer<I>::value || CPP_requires_ref(detail::has_member_arrow_, I));
            // clang-format on
        } // namespace detail
        /// \endcond
        ...
        template<typename Rng>
        struct EARANGES_EMPTY_BASES join_view
          ...
          , private detail::join_view_inner<Rng>
        {
            ...
            constexpr CPP_ctor(join_view)()(                                  //
                noexcept(eastl::is_nothrow_default_constructible<Rng>::value) //
                requires default_constructible<Rng>)
            {}

            explicit join_view(Rng rng)
              : outer_(views::all(eastl::move(rng)))
            {}
            // Not to spec
            CPP_member
            static constexpr auto size() //
                -> CPP_ret(std::size_t)(requires(detail::join_cardinality<Rng>() >= 0))
            {
                ...
                sized_range<range_reference_t<Rng>>)
            {
                ...
                EARANGES_FOR(auto && inner, outer_)
                n += ranges::size(inner);
                return n;
            }
        ...
        private:
            ...
            private:
                ...
                using ref_is_glvalue = eastl::is_reference<CInner>;
                ...
                void satisfy()
                {
                    ...
                    if(EARANGES_CONSTEXPR_IF(ref_is_glvalue::value))
            ...
            public:
                ...
                template(bool Other)(
                    requires Const AND CPP_NOT(Other)
                        AND convertible_to<iterator_t<Rng>, iterator_t<COuter>>
                            AND convertible_to<
                                iterator_t<range_reference_t<Rng>>,
                                iterator_t<CInner>>) constexpr cursor(cursor<Other> that)
                  : rng_(that.rng_)
                  , outer_it_(eastl::move(that.outer_it_))
                  , inner_it_(eastl::move(that.inner_it_))
                {}
                CPP_member
                ...
                constexpr auto equal(cursor const & that) const                     //
                    -> CPP_ret(bool)(requires ref_is_glvalue::value &&              //
            ...
            {
                return simple_view<Rng>() &&
                       eastl::is_reference<range_reference_t<Rng>>::value;
            }
            struct end_cursor_fn
            {
                constexpr auto operator()(join_view * this_, eastl::true_type) const
                {
                    return cursor<use_const_always()>{this_, ranges::end};
                }
                constexpr auto operator()(join_view *, eastl::false_type) const
                {
            ...
            {
                constexpr auto operator()(join_view const * this_, eastl::true_type) const
                {
                    return cursor<true>{this_, ranges::end};
                }
                constexpr auto operator()(join_view const *, eastl::false_type) const
                {
                    return default_sentinel_t{};
                }
            };
            ...
            template(bool Const = true)(
                requires Const AND input_range<meta::const_if_c<Const, Rng>> AND
                    eastl::is_reference<range_reference_t<meta::const_if_c<Const, Rng>>>::
                        value) constexpr cursor<Const> begin_cursor() const
            {
                return {this, ranges::begin};
            }
            ...
            template(bool Const = true)(
                requires Const AND input_range<meta::const_if_c<Const, Rng>> AND
                    eastl::is_reference<range_reference_t<meta::const_if_c<Const, Rng>>>::
                        value) constexpr auto end_cursor() const
            {
                using CRng = meta::const_if_c<Const, Rng>;
                using cond = meta::bool_<
                    eastl::is_reference<range_reference_t<CRng>>::value &&
        ...
        struct join_with_view
          : view_facade<join_with_view<Rng, ValRng>,
          ...
          , private detail::join_view_inner<Rng>
        {
            ...
            CPP_assert(common_with<range_value_t<range_reference_t<Rng>>,
            ...
            public:
                ...
                void next()
                {
                    ...
                    if(cur_.index() == 0)
                    {
                        ...
                        EARANGES_ASSERT(it != ranges::end(rng_->val_));
                        ++it;
                    }
                    else
                    {
...
#ifndef NDEBUG
                        auto && inner = rng_->get_inner_(outer_it_);
                        EARANGES_ASSERT(it != ranges::end(inner));
#endif
                        ++it;
                    }
                    satisfy();
        ...
        CPP_concept joinable_with_range =
            ...
            {
                template(typename Rng)(requires joinable_range<Rng>) join_view<all_t<Rng>>
            ...
            public:
                ...
                {
                    return {all(static_cast<Rng &&>(rng)), single(eastl::move(v))};
                }

                template(typename Rng,
                         typename ValRng)(requires joinable_with_range<Rng, ValRng>)
                    join_with_view<all_t<Rng>, all_t<ValRng>> //
                    operator()(Rng && rng, ValRng && val) const
                {
                    return {all(static_cast<Rng &&>(rng)),
                            all(static_cast<ValRng &&>(val))};
                }
            ...
            struct join_bind_fn
            {
                    ...
                    operator()(T && t) const
                {
                    return make_view_closure(
                        bind_back(join_base_fn{}, static_cast<T &&>(t)));
                }
                template(typename T)(requires(!joinable_range<T &>)
                                         AND range<T &>) constexpr auto
                operator()(T & t) const
                {
                    return make_view_closure(
                        bind_back(join_base_fn{}, detail::reference_wrapper_<T>(t)));
                }
            };

            struct EARANGES_EMPTY_BASES join_fn
              : join_base_fn
              , join_bind_fn
            {
            ...
            /// \relates join_fn
            /// \ingroup group-views
            EARANGES_INLINE_VARIABLE(view_closure<join_fn>, join)
        } // namespace views
        /// @}

#if EARANGES_CXX_DEDUCTION_GUIDES >= EARANGES_CXX_DEDUCTION_GUIDES_17
        template(typename Rng)(requires views::joinable_range<Rng>) explicit join_view(
            Rng &&) -> join_view<views::all_t<Rng>>;

        template(typename Rng,
                 typename ValRng)(requires views::joinable_with_range<
                                  Rng, ValRng>) explicit join_with_view(Rng &&, ValRng &&)
...
EARANGES_SATISFY_BOOST_RANGE(::ranges::join_view)
EARANGES_SATISFY_BOOST_RANGE(::ranges::join_with_view)
