...
#ifndef EARANGES_VIEW_ANY_VIEW_HPP
#define EARANGES_VIEW_ANY_VIEW_HPP
...
EARANGES_DIAGNOSTIC_IGNORE_INCONSISTENT_OVERRIDE
EARANGES_DIAGNOSTIC_SUGGEST_OVERRIDE

namespace eastl
{
    namespace ranges
    {
        /// \brief An enum that denotes the supported subset of range concepts supported
        /// by a range.
        enum class category
        {
        ...
        constexpr category operator&(category lhs, category rhs) noexcept
        {
            return static_cast<category>(
                static_cast<meta::_t<eastl::underlying_type<category>>>(lhs) &
                static_cast<meta::_t<eastl::underlying_type<category>>>(rhs));
        }
        ...
        namespace detail
        {
            ...
            struct any_ref
            {
                ...
                {
                    EARANGES_ASSERT(obj_ && info_ && *info_ == typeid(rtti_tag<T>));
            ...
            struct cloneable : Base
            {
                ...
                virtual eastl::unique_ptr<cloneable> clone() const = 0;
            };
        ...
        CPP_concept any_compatible_range =
            ...
            template<typename Rng>
            struct any_view_sentinel_impl<
                Rng, meta::void_<decltype(ranges::end(eastl::declval<Rng const &>()))>>
            {
                any_view_sentinel_impl() = default;
            ...
            struct EARANGES_EMPTY_BASES any_input_view_impl
            ...
            private:
                ...
                EARANGES_NO_UNIQUE_ADDRESS iterator_t<Rng> current_{};
            };
            ...
            struct any_cursor_interface<
              ...
              : any_cursor_interface<Ref,
                                     (Cat & ~category::mask) | category::bidirectional>
            {
                virtual void advance(std::ptrdiff_t) = 0;
                virtual std::ptrdiff_t distance_to(
                    any_cursor_interface const &) const = 0;
            };
            ...
            private:
                using Forward = any_cursor_interface<Ref, (Cat & ~category::mask) |
                                                              category::forward>;
                ...
                eastl::unique_ptr<any_cloneable_cursor_interface<Ref, Cat>> clone()
                    const override
                {
            ...
            private:
                ...
                eastl::unique_ptr<any_cloneable_cursor_interface<Ref, Cat>> ptr_;
            ...
            public:
                any_cursor() = default;
                template(typename Rng)(
                    requires(!same_as<detail::decay_t<Rng>, any_cursor>)
                        AND forward_range<Rng>
                            AND any_compatible_range<Rng,
                                                     Ref>) explicit any_cursor(Rng && rng)
            ...
            struct EARANGES_EMPTY_BASES any_view_impl
            ...
            private:
                ...
                eastl::unique_ptr<any_cloneable_view_interface<Ref, Cat>> clone()
                    const override
                {
                        ...
                        (Cat & category::sized) == category::sized ? finite : unknown>
        {
            ...
            CPP_member
            auto size() //
                -> CPP_ret(std::size_t)(requires(category::sized ==
                                                 (Cat & category::sized)))
            {
        ...
        private:
            ...
            template<typename Rng>
            any_view(Rng && rng, eastl::true_type)
              : ptr_{detail::make_unique<impl_t<Rng>>(
                    views::all(static_cast<Rng &&>(rng)))}
            {}
            template<typename Rng>
            any_view(Rng &&, eastl::false_type)
            {
            ...
            eastl::unique_ptr<detail::any_cloneable_view_interface<Ref, Cat>> ptr_;
        };

        // input and not forward
        template<typename Ref, category Cat>
        struct any_view<Ref, Cat,
                        ...
                        (Cat & category::sized) == category::sized ? finite : unknown>
        {
            ...
            template(typename Rng)(requires //
                ...
                any_view(Rng && rng)
              : ptr_{
                    eastl::make_shared<impl_t<Rng>>(views::all(static_cast<Rng &&>(rng)))}
            {}
        ...
        private:
            ...
            eastl::shared_ptr<detail::any_input_view_interface<
                Ref, (Cat & category::sized) == category::sized>>
                ptr_;
        };
