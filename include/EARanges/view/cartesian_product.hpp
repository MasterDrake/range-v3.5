    ...
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            using product_cardinality = eastl::integral_constant<
                cardinality,
                State::value == 0 || Value::value == 0 ? static_cast<cardinality>(0)
            ...
            using cartesian_product_cardinality = meta::fold<
                ...
                eastl::integral_constant<cardinality, static_cast<cardinality>(
                                                          (sizeof...(Views) > 0))>,
    ...
    CPP_concept cartesian_produce_view_can_bidi =
        ...
        private:
            ...
            private:
                ...
                {
                    return difference_type{eastl::get<0>(that.its_) -
                                           eastl::get<0>(its_)};
                }
                template<std::size_t N>
                difference_type distance_(cursor const & that, meta::size_t<N>) const
                {
                    ...
                    auto const increment =
                        eastl::get<N - 1>(that.its_) - eastl::get<N - 1>(its_);
                ...
                {
                    if(at_end)
                        ranges::advance(eastl::get<0>(its_),
                                        ranges::end(eastl::get<0>(view_->views_)));
                }
                template<std::size_t N>
                void check_at_end_(meta::size_t<N>, bool at_end = false)
                {
                    return check_at_end_(
                        meta::size_t<N - 1>{},
                        at_end || bool(eastl::get<N - 1>(its_) ==
                                       ranges::end(eastl::get<N - 1>(view_->views_))));
                }
                cursor(end_tag, constify_if<cartesian_product_view> * view,
                       eastl::true_type) // common_with
            ...
            public:
                ...
                explicit cursor(end_tag, constify_if<cartesian_product_view> * view)
                  : cursor(end_tag{}, view,
                           meta::bool_<common_range<
                               meta::at_c<meta::list<constify_if<Views>...>, 0>>>{})
                {}
                template(bool Other)(requires IsConst_ AND CPP_NOT(Other)) //
                ...
                CPP_auto_member auto CPP_fun(distance_to)(cursor const & that)(
            ...
            auto end_cursor() //
                -> CPP_ret(cursor<false>)(
                    requires cartesian_produce_view_can_bidi<eastl::false_type, Views...>)
            {
            ...
            auto end_cursor() const //
                -> CPP_ret(cursor<true>)(
                    requires cartesian_produce_view_can_bidi<eastl::true_type, Views...>)
            {
                return cursor<true>{end_tag{}, this};
            }
            CPP_member
            auto end_cursor() const //
                -> CPP_ret(default_sentinel_t)(requires(
                    !cartesian_produce_view_can_bidi<eastl::true_type, Views...>))
            {
                return {};
            }
        ...
        public:
            ...
            {
                return tuple_foldl(
                    views_, std::uintmax_t{1}, detail::cartesian_size_fn{});
            }
            CPP_auto_member auto CPP_fun(size)()(
                requires(my_cardinality < 0) &&
        ...
        namespace views
        {
            struct cartesian_product_fn
            {
                ...
                template(typename... Rngs)(
                    requires(sizeof...(Rngs) != 0) AND concepts::and_v<
                        (forward_range<Rngs> &&
                         viewable_range<
                             Rngs>)...>) constexpr cartesian_product_view<all_t<Rngs>...>
                ...
                template(typename Rng0)(
                    requires forward_range<Rng0> AND viewable_range<
                        Rng0>) constexpr cartesian_product_view<all_t<Rng0>>
                operator()(Rng0 && rng0) const
                {
                ...
                template(typename Rng0, typename Rng1)(
                    requires forward_range<Rng0> AND viewable_range<Rng0> AND
                        forward_range<Rng1>
                            AND viewable_range<
                                Rng1>) constexpr cartesian_product_view<all_t<Rng0>,
                                                                        all_t<Rng1>> //
                ...
                template(typename Rng0, typename Rng1, typename Rng2)(
                    requires forward_range<Rng0> AND viewable_range<Rng0> AND forward_range<
                        Rng1>
                        AND viewable_range<Rng1>
                            AND forward_range<Rng2>
                                AND viewable_range<
                                    Rng2>) constexpr cartesian_product_view<all_t<Rng0>,
                                                                            all_t<Rng1>,
                                                                            all_t<
                                                                                Rng2>> //
