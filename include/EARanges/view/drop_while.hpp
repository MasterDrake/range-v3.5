...
namespace eastl
{
    namespace ranges
    {
        ...
        template<typename Rng, typename Pred>
        EARANGES_INLINE_VAR constexpr bool
            disable_sized_range<drop_while_view<Rng, Pred>> = true;
            ...
            struct drop_while_base_fn
            {
                template(typename Rng, typename Pred)(
                    requires viewable_range<Rng> AND input_range<Rng> AND
                        indirect_unary_predicate<Pred, iterator_t<Rng>>) auto
                ...
                template(typename Rng, typename Pred, typename Proj)(
                    requires viewable_range<Rng> AND input_range<Rng> AND
                        indirect_unary_predicate<composed<Pred, Proj>,
                                                 iterator_t<Rng>>) auto
            ...
            struct drop_while_bind_fn
            {
                    ...
                    Pred pred) const // TODO: underconstrained - Eric Niebler
                {
                    return make_view_closure(
                        bind_back(drop_while_base_fn{}, eastl::move(pred)));
                }
                ...
                {
                    return make_view_closure(bind_back(drop_while_base_fn{},
                                                       static_cast<Pred &&>(pred),
                                                       eastl::move(proj)));
                }
            };

            struct EARANGES_EMPTY_BASES drop_while_fn
              : drop_while_base_fn
              , drop_while_bind_fn
            {
