    ...
    EARANGES_CXX_VER <= EARANGES_CXX_STD_17
...
#if CPP_CXX_CONCEPTS
            requires input_range<V> && forward_range<Pattern> && view_<V> &&
                     view_<Pattern> &&
        ...
        namespace detail
        {
            ...
            private:
                ...
                using BaseIterCategory =
                    typename eastl::iterator_traits<iterator_t<Base>>::iterator_category;
                Outer i_ = Outer();
            ...
            public:
                ...
                using iterator_category = meta::conditional_t<
                    ...
                    eastl::forward_iterator_tag, eastl::input_iterator_tag>;
                ...
                friend constexpr auto operator==(split_inner_iterator const & x,
                    ...
                    -> CPP_broken_friend_ret(bool)(requires forward_range<Base>)
                {
                ...
                friend constexpr auto iter_swap(
                    split_inner_iterator const & x,
                    split_inner_iterator const &
                        y) noexcept(noexcept(ranges::iter_swap(x.current_(),
                                                               y.current_())))
            ...
            public:
                using iterator_concept =
                    meta::conditional_t<forward_range<Base>, eastl::forward_iterator_tag,
                ...
                template(bool Other)(
                    requires Const AND CPP_NOT(Other) AND convertible_to<
                        iterator_t<V>,
                        iterator_t<
                            Base>>) constexpr split_outer_iterator(split_outer_iterator<split_view<V,
                                                                                                   Pattern>,
                                                                                        Other>
                                                                       i)
                ...
                constexpr split_outer_iterator & operator++()
                {
                    ...
                    if(pbegin == pend)
                        ...
                        {
                            const auto ret =
                                ranges::mismatch(current, last, pbegin, pend);
                            if(ret.in2 == pend)
                            {
        ...
        template(typename R, typename P)(
            requires input_range<R> AND forward_range<P> AND viewable_range<R> AND
                viewable_range<P>
                    AND indirectly_comparable<iterator_t<R>, iterator_t<P>,
                                              ranges::equal_to>
                        AND(forward_range<R> || detail::tiny_range<P>)) //
            split_view(R &&, P &&) -> split_view<views::all_t<R>, views::all_t<P>>;

        template(typename R)(requires input_range<R>) split_view(R &&, range_value_t<R>)
                ...
                template(typename Rng)(
                    requires viewable_range<Rng> AND input_range<Rng> AND
                        indirectly_comparable<
                            iterator_t<Rng>, range_value_t<Rng> const *,
                            ranges::equal_to>) constexpr split_view<all_t<Rng>,
                                                                    single_view<
                                                                        range_value_t<
                                                                            Rng>>> //
