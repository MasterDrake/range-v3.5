...
#ifndef EARANGES_VIEW_SLIDING_HPP
#define EARANGES_VIEW_SLIDING_HPP
...
namespace ranges
{
    ...
    namespace sliding_view_detail
    {
        ...
        using caching = eastl::integral_constant<
    ...
    template<typename Rng, sliding_view_detail::cache = sliding_view_detail::caching<Rng>::value>
    struct sliding_view;
    ...
    {
        template<typename Rng>
        using uncounted_t = decltype(ranges::uncounted(eastl::declval<iterator_t<Rng> &>()));

        template<typename Rng, bool = (bool)random_access_range<Rng>>
        struct trailing
        {
            trailing() = default;
            constexpr trailing(Rng & rng) : it_{uncounted(ranges::begin(rng))}
            {}
            constexpr uncounted_t<Rng> get(iterator_t<Rng> const &, range_difference_t<Rng>) const
            {
                return it_;
            }
            void next()
        ...
        struct trailing<Rng, true>
        {
            ...
            {}
            constexpr uncounted_t<Rng> get(iterator_t<Rng> const & it, range_difference_t<Rng> n) const
            {
        ...
        template<typename Rng>
        struct EARANGES_EMPTY_BASES sv_base
          : view_adaptor<sliding_view<Rng>, Rng, is_finite<Rng>::value ? finite : range_cardinality<Rng>::value>
          , private detail::non_propagating_cache<iterator_t<Rng>, sv_base<Rng>, caching<Rng>::value != cache::none>
        {
            CPP_assert(forward_range<Rng>);
            sv_base() = default;
            sv_base(Rng rng, range_difference_t<Rng> n)
              : sv_base::view_adaptor(eastl::move(rng))
              , n_(n)
            {
                EARANGES_ASSERT(0 < n_);
            }
            CPP_auto_member
    ...
    /// @{
    template<typename Rng>
    struct sliding_view<Rng, sliding_view_detail::cache::first> : sliding_view_detail::sv_base<Rng>
    {
    private:
        friend range_access;
        ...
        iterator_t<Rng> get_first()
        {
            ...
            if(!first)
            {
                first = ranges::next(ranges::begin(this->base()), this->n_ - 1, ranges::end(this->base()));
            }
            return *first;
        }

        struct EARANGES_EMPTY_BASES adaptor
          : adaptor_base
        ...
        public:
            ...
            auto advance(iterator_t<Rng> & it, range_difference_t<Rng> n)
                -> CPP_ret(void)(requires random_access_range<Rng>)
            {
                it += n;
            }
        };
    ...
    template<typename Rng>
    struct sliding_view<Rng, sliding_view_detail::cache::last> : sliding_view_detail::sv_base<Rng>
    {
    private:
        friend range_access;
        ...
        iterator_t<Rng> get_last()
        {
            ...
            if(!last)
            {
                last = ranges::prev(ranges::end(this->base()), this->n_ - 1, ranges::begin(this->base()));
            }
            return *last;
        }
        ...
        public:
            adaptor() = default;
            adaptor(sliding_view * v) : n_{v->n_}
            {}
            iterator_t<Rng> end(sliding_view & v)
            {
                return v.get_last();
            }
            auto read(iterator_t<Rng> const & it) const -> decltype(views::counted(uncounted(it), n_))
            {
    ...
    template<typename Rng>
    struct sliding_view<Rng, sliding_view_detail::cache::none> : sliding_view_detail::sv_base<Rng>
    {
    private:
        friend range_access;
        ...
        public:
            ...
            adaptor(adaptor<Other> that) : n_(that.n_)
            {}
    ...
    template<typename Rng>
    EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<sliding_view<Rng>> = //
    ...
    namespace views
    {
        ...
        /// \relates sliding_fn
        /// \ingroup group-views
        EARANGES_INLINE_VARIABLE(sliding_fn, sliding)
    } // namespace views
    /// @}
} // namespace ranges
