    ...
    namespace ranges
    {
        ...
        private:
            ...
            private:
                ...
                EARANGES_NO_UNIQUE_ADDRESS semiregular_box_ref_or_val_t<Pred, IsConst>
                    pred_;
            ...
            public:
                ...
                {}
                bool empty(iterator_t<CRng> const & it,
                           sentinel_t<CRng> const & last) const
                {
        ...
        struct take_while_view : iter_take_while_view<Rng, indirected<Pred>>
        {
            ...
            constexpr take_while_view(Rng rng, Pred pred)
              : iter_take_while_view<Rng, indirected<Pred>>{eastl::move(rng),
                                                            indirect(eastl::move(pred))}
            {}
        };
        ...
        namespace views
        {
                    ...
                    requires viewable_range<Rng> AND input_range<Rng> AND
                        predicate<Pred &, iterator_t<Rng>>
                            AND copy_constructible<
                                Pred>) constexpr iter_take_while_view<all_t<Rng>, Pred> //
            ...
            struct iter_take_while_fn : iter_take_while_base_fn
            {
                ...
                constexpr auto operator()(Pred pred) const
                {
                    return make_view_closure(
                        bind_back(iter_take_while_base_fn{}, eastl::move(pred)));
                }
            };
                ...
                template(typename Rng, typename Pred)(
                    requires viewable_range<Rng> AND input_range<Rng> AND
                        indirect_unary_predicate<
                            Pred &,
                            iterator_t<Rng>>) constexpr take_while_view<all_t<Rng>,
                                                                        Pred> //
                operator()(Rng && rng, Pred pred) const
                {
                    return {all(static_cast<Rng &&>(rng)), eastl::move(pred)};
            ...
            struct take_while_bind_fn
            {
                ...
                {
                    return make_view_closure(
                        bind_back(take_while_base_fn{}, eastl::move(pred)));
                }
                ...
                {
                    return make_view_closure(bind_back(take_while_base_fn{},
                                                       static_cast<Pred &&>(pred),
                                                       eastl::move(proj)));
                }
            };

            struct EARANGES_EMPTY_BASES take_while_fn
              : take_while_base_fn
              , take_while_bind_fn
            {
