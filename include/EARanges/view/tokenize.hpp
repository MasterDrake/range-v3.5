...
#ifndef EARANGES_VIEW_TOKENIZE_HPP
#define EARANGES_VIEW_TOKENIZE_HPP
    ...
    namespace ranges
    {
        ...
        namespace views
        {
            struct tokenize_base_fn
            {
                ...
                template(typename Rng, typename Regex)(
                    requires bidirectional_range<Rng> AND common_range<Rng> AND same_as<
                        range_value_t<Rng>, typename detail::decay_t<Regex>::value_type>)
                    tokenize_view<all_t<Rng>, detail::decay_t<Regex>,
                                  eastl::vector<int>> //
                    operator()(Rng && rng, Regex && rex, eastl::vector<int> subs,
                               std::regex_constants::match_flag_type flags =
                                   std::regex_constants::match_default) const //
                {
                    return {all(static_cast<Rng &&>(rng)),
            ...
            /// \relates tokenize_fn
            /// \ingroup group-views
            EARANGES_INLINE_VARIABLE(tokenize_fn, tokenize)
        } // namespace views
        /// @}
    } // namespace ranges
} // namespace eastl
...
EARANGES_SATISFY_BOOST_RANGE(::ranges::tokenize_view)
