    ...
    namespace ranges
    {
        ...
        private:
            ...
            public:
                ...
                template(bool Other)(
                    requires IsConst AND CPP_NOT(Other) AND convertible_to<
                        iterator_t<Rng> const &,
                        iterator_t<Base>>) constexpr cursor(cursor<Other> const & that)
                ...
                CPP_auto_member constexpr bool CPP_fun(equal)(cursor const & that)(
                    const //
            ...
            template(typename CRng = Rng const)(
                requires detail::partial_sum_view_constraints<
                    CRng, Fun const>) constexpr cursor<true> begin_cursor() const
            {
        ...
        namespace views
        {
            struct partial_sum_base_fn
            {
                template(typename Rng, typename Fun = plus)(
                    requires detail::partial_sum_view_constraints<
                        all_t<Rng>, Fun>) constexpr partial_sum_view<all_t<Rng>, Fun> //
