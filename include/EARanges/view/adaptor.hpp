...
//
#ifndef EARANGES_VIEW_ADAPTOR_HPP
#define EARANGES_VIEW_ADAPTOR_HPP
...
namespace ranges
{
    ...
    namespace detail
    {
        ...
        using begin_adaptor_t = detail::decay_t<decltype(range_access::begin_adaptor(eastl::declval<Derived &>()))>;

        template<typename Derived>
        using end_adaptor_t = detail::decay_t<decltype(range_access::end_adaptor(eastl::declval<Derived &>()))>;

        template<typename Derived>
        using adapted_iterator_t = detail::decay_t<decltype(eastl::declval<begin_adaptor_t<Derived>>().begin(eastl::declval<Derived &>()))>;

        template<typename Derived>
        using adapted_sentinel_t = detail::decay_t<decltype( eastl::declval<end_adaptor_t<Derived>>().end(eastl::declval<Derived &>()))>;
        ...
        template<typename BaseIter, typename Adapt>
        constexpr always_<int, decltype(Adapt::read(eastl::declval<BaseIter const &>(), adaptor_base_current_mem_fn{}))> //
        ...
        {
#ifdef EARANGES_WORKAROUND_MSVC_688606
    ...
    struct adaptor_base
    {
        ...
        (
            ...
            // clang-format on
            template(typename I)(requires equality_comparable<I>)
            static bool equal(I const & it0, I const & it1)
        {
            return it0 == it1;
        }
        template(typename I)(requires input_or_output_iterator<I>)
        static iter_reference_t<I> read(I const & it, detail::adaptor_base_current_mem_fn = {})
    ...
    public:
        ...
        base_adaptor_sentinel(BaseSent sent, Adapt adapt)
          : data_{eastl::move(sent), eastl::move(adapt)}
        {}
    ...
    private:
        ...
        using single_pass = meta::bool_<(bool)range_access::single_pass_t<Adapt>() || (bool)single_pass_iterator_<BaseIter>>;
        ...
        template<typename A = Adapt, typename R = decltype(eastl::declval<A const &>().read(eastl::declval<BaseIter const &>()))>
        R read() const noexcept(
            noexcept(eastl::declval<A const &>().read(eastl::declval<BaseIter const &>())))
        {
        ...
        template<typename A = Adapt, typename = decltype(eastl::declval<A &>().next(eastl::declval<BaseIter &>()))>
        void next()
        {
            this->data_.second().next(this->data_.first());
        }
        template<typename A = Adapt,
                 typename = decltype(eastl::declval<A const &>().equal(
                     eastl::declval<BaseIter const &>(), eastl::declval<BaseIter const &>(),
                     eastl::declval<A const &>()))>
        ...
        template<typename A = Adapt, typename = decltype(eastl::declval<A const &>().equal(eastl::declval<BaseIter const &>(), eastl::declval<BaseIter const &>()))>
        ...
        auto equal(adaptor_cursor const & that) const -> decltype(eastl::declval<C const &>().equal_(that, 42))
        {
            return this->equal_(that, 42);
        }
        template<typename S, typename A,
                 typename = decltype(eastl::declval<A const &>().empty(
                     eastl::declval<BaseIter const &>(), eastl::declval<Adapt const &>(),
                     eastl::declval<S const &>()))>
        ...
        template<typename S, typename A,
                 typename = decltype(eastl::declval<A const &>().empty(
                     eastl::declval<BaseIter const &>(), eastl::declval<S const &>()))>
        ...
        constexpr auto equal(adaptor_sentinel<S, A> const & that) const
            -> decltype(eastl::declval<adaptor_cursor const &>().equal_(that, 42))
        {
            return this->equal_(that, 42);
        }
        template<typename A = Adapt, typename = decltype(eastl::declval<A &>().prev(
                                         eastl::declval<BaseIter &>()))>
        void prev()
        {
            this->data_.second().prev(this->data_.first());
        }
        template<typename A = Adapt, typename = decltype(eastl::declval<A &>().advance(
                                         eastl::declval<BaseIter &>(), 0))>
        void advance(iter_difference_t<BaseIter> n)
        {
            this->data_.second().advance(this->data_.first(), n);
        }
        template<typename A = Adapt,
                 typename R = decltype(eastl::declval<A const &>().distance_to(
                     eastl::declval<BaseIter const &>(), eastl::declval<BaseIter const &>(),
                     eastl::declval<A const &>()))>
        ...
        auto distance_to(adaptor_cursor const & that) const
            -> decltype(eastl::declval<C const &>().distance_to_(that, 42))
        {
            return this->distance_to_(that, 42);
        }
        // If the adaptor has an iter_move function, use it.
        template<typename A = Adapt,
                 typename X = decltype(eastl::declval<A const &>().iter_move(
                     eastl::declval<BaseIter const &>()))>
        X iter_move_(int) const noexcept(noexcept(
            eastl::declval<A const &>().iter_move(eastl::declval<BaseIter const &>())))
        {
            using V = range_access::cursor_value_t<adaptor_cursor>;
        ...
        template<typename A = Adapt,
                 typename R = decltype(eastl::declval<A const &>().read(
                     eastl::declval<BaseIter const &>(),
                     detail::adaptor_base_current_mem_fn{})),
                 typename X = iter_rvalue_reference_t<BaseIter>>
        X iter_move_(long) const
            noexcept(noexcept(X(ranges::iter_move(eastl::declval<BaseIter const &>()))))
        {
        ...
        auto move() const
            noexcept(noexcept(eastl::declval<const adaptor_cursor &>().iter_move_(42)))
                -> decltype(eastl::declval<const adaptor_cursor &>().iter_move_(42))
        {
    ...
    public:
        ...
        adaptor_cursor(BaseIter iter, Adapt adapt)
          : base_t{{eastl::move(iter), eastl::move(adapt)}}
        {}
        ...
        adaptor_cursor(adaptor_cursor<OtherIter, OtherAdapt> that)
          : base_t{{eastl::move(that.data_.first()), eastl::move(that.data_.second())}}
        {}
    };
    ...
    private:
        ...
        static constexpr adaptor_cursor_t<D> begin_cursor_(D & d) noexcept(noexcept(
            adaptor_cursor_t<D>{eastl::declval<detail::begin_adaptor_t<D> &>().begin(d),
                                range_access::begin_adaptor(d)}))
        {
            auto adapt = range_access::begin_adaptor(d);
            auto pos = adapt.begin(d);
            return {eastl::move(pos), eastl::move(adapt)};
        }
        template(typename D = Derived)(
            requires same_as<D, Derived>)
        constexpr auto begin_cursor() noexcept(
            noexcept(view_adaptor::begin_cursor_(eastl::declval<D &>())))
            -> decltype(view_adaptor::begin_cursor_(eastl::declval<D &>()))
        {
        ...
        constexpr auto begin_cursor() const
            noexcept(noexcept(view_adaptor::begin_cursor_(eastl::declval<D const &>())))
                -> decltype(view_adaptor::begin_cursor_(eastl::declval<D const &>()))
        {
            return view_adaptor::begin_cursor_(derived());
        }
        ...
        static constexpr adaptor_sentinel_t<D> end_cursor_(D & d) noexcept(noexcept(
            adaptor_sentinel_t<D>{eastl::declval<detail::end_adaptor_t<D> &>().end(d),
        ...
        constexpr auto end_cursor() noexcept(
            noexcept(view_adaptor::end_cursor_(eastl::declval<D &>())))
            -> decltype(view_adaptor::end_cursor_(eastl::declval<D &>()))
        {
        ...
        constexpr auto end_cursor() const noexcept(
            noexcept(view_adaptor::end_cursor_(eastl::declval<D const &>())))
            -> decltype(view_adaptor::end_cursor_(eastl::declval<D const &>()))
        {
            return view_adaptor::end_cursor_(derived());
        }
    ...
    public:
        ...
        constexpr explicit view_adaptor(BaseRng && rng): rng_(views::all(static_cast<BaseRng &&>(rng)))
        {}
