...
#ifndef EARANGES_VIEW_CONCAT_HPP
#define EARANGES_VIEW_CONCAT_HPP
...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            using concat_cardinality_ = eastl::integral_constant<
                cardinality,
                State::value == infinite || Value::value == infinite ? infinite
            ...
            using concat_cardinality = meta::fold<
                ...
                eastl::integral_constant<cardinality, static_cast<cardinality>(0)>,
        ...
        private:
            ...
            public:
                ...
                sentinel(concat_view_t * rng, end_tag)
                  : end_(end(eastl::get<cranges - 1>(rng->rngs_)))
                {}
                template(bool Other)(requires IsConst AND CPP_NOT(Other)) //
            ...
            private:
                ...
                void satisfy(meta::size_t<N>)
                {
                    ...
                    if(ranges::get<N>(its_) == end(eastl::get<N>(rng_->rngs_)))
                    {
                        ranges::emplace<N + 1>(its_,
                                               begin(eastl::get<N + 1>(rng_->rngs_)));
                        this->satisfy(meta::size_t<N + 1>{});
                    }
                }
                void satisfy(meta::size_t<cranges - 1>)
                {
                    EARANGES_EXPECT(its_.index() == cranges - 1);
                }
                struct next_fun
                {
                    cursor * pos;
                    template(typename I, std::size_t N)(requires input_iterator<I>) void
                    operator()(indexed_element<I, N> it) const
                    {
                        EARANGES_ASSERT(it.get() != end(eastl::get<N>(pos->rng_->rngs_)));
                        ++it.get();
                ...
                {
                    cursor * pos;
                    template(typename I)(requires bidirectional_iterator<I>) void
                    operator()(indexed_element<I, 0> it) const
                    {
                        EARANGES_ASSERT(it.get() !=
                                        begin(eastl::get<0>(pos->rng_->rngs_)));
                        --it.get();
                    }
                    template(typename I,
                             std::size_t N)(requires(N != 0)
                                                AND bidirectional_iterator<I>) void
                    operator()(indexed_element<I, N> it) const
                    {
                        if(it.get() == begin(eastl::get<N>(pos->rng_->rngs_)))
                        {
                            auto && rng = eastl::get<N - 1>(pos->rng_->rngs_);
                            ranges::emplace<N - 1>(
                                pos->its_,
                ...
                struct advance_fwd_fun
                {
                    cursor * pos;
                    difference_type n;
                    template(typename I)(requires random_access_iterator<I>) void
                    operator()(indexed_element<I, cranges - 1> it) const
                    {
                        ranges::advance(it.get(), n);
                    }
                    template(typename I,
                             std::size_t N)(requires random_access_iterator<I>) void
                    operator()(indexed_element<I, N> it) const
                    {
                        auto last = ranges::end(eastl::get<N>(pos->rng_->rngs_));
                        ...
                        auto rest = ranges::advance(it.get(), n, eastl::move(last));
                        pos->satisfy(meta::size_t<N>{});
                        if(rest != 0)
                ...
                struct advance_rev_fun
                {
                    ...
                    operator()(indexed_element<I, N> it) const
                    {
                        auto first = ranges::begin(eastl::get<N>(pos->rng_->rngs_));
                        if(it.get() == first)
                        {
                            auto && rng = eastl::get<N - 1>(pos->rng_->rngs_);
                            ranges::emplace<N - 1>(
                        ...
                        {
                            auto rest = ranges::advance(it.get(), n, eastl::move(first));
                            if(rest != 0)
                                                    ...
                                                    cursor const & to)
                {
                    ...
                    if(from.its_.index() == N)
                    {
                        ...
                        return distance(ranges::get<N>(from.its_),
                                        end(eastl::get<N>(from.rng_->rngs_))) +
                               cursor::distance_to_(meta::size_t<N + 1>{}, from, to);
                    }
                    if(from.its_.index() < N && to.its_.index() > N)
                        return distance(eastl::get<N>(from.rng_->rngs_)) +
                               cursor::distance_to_(meta::size_t<N + 1>{}, from, to);
                    EARANGES_EXPECT(to.its_.index() == N);
                    return distance(begin(eastl::get<N>(from.rng_->rngs_)),
                                    ranges::get<N>(to.its_));
                }
            ...
            public:
                ...
                using reference =
                    common_reference_t<range_reference_t<constify_if<Rngs>>...>;
                using single_pass =
                    meta::or_c<single_pass_iterator_<iterator_t<Rngs>>...>;
                cursor() = default;
                cursor(concat_view_t * rng, begin_tag)
                  : rng_(rng)
                  , its_{emplaced_index<0>, begin(eastl::get<0>(rng->rngs_))}
                {
                    this->satisfy(meta::size_t<0>{});
                }
                cursor(concat_view_t * rng, end_tag)
                  : rng_(rng)
                  , its_{emplaced_index<cranges - 1>,
                         end(eastl::get<cranges - 1>(rng->rngs_))}
                {}
                ...
                CPP_member
                auto prev() //
                    -> CPP_ret(void)(requires and_v<bidirectional_range<Rngs>...>)
                {
                    its_.visit_i(prev_fun{this});
                }
                CPP_member
                auto advance(difference_type n) //
                    -> CPP_ret(void)(requires and_v<random_access_range<Rngs>...>)
                {
                    if(n > 0)
            ...
            auto begin_cursor() const //
                -> CPP_ret(cursor<true>)(requires and_v<range<Rngs const>...>)
            {
                return {this, begin_tag{}};
            }
            CPP_member
            auto end_cursor() const                                                  //
                -> CPP_ret(meta::if_<meta::and_c<(bool)common_range<Rngs const>...>, //
                                     cursor<true>,                                   //
                                     sentinel<true>>)(
        ...
        public:
                ...
                and_v<sized_range<Rngs const>...>)
            {
                ...
                return tuple_foldl(
                    tuple_transform(
                        rngs_, [](auto && r) -> size_type { return ranges::size(r); }),
                    size_type{0},
                    plus{});
            }
            CPP_auto_member constexpr auto CPP_fun(size)()(
        ...
        namespace views
        {
            struct concat_fn
            {
                ...
                {
                    return concat_view<all_t<Rngs>...>{
                        all(static_cast<Rngs &&>(rngs))...};
                }
                template(typename Rng)(
                    requires viewable_range<Rng> AND input_range<Rng>) all_t<Rng>
                ...
                template(typename Rng0, typename Rng1)(
                    requires viewable_range<Rng0> AND input_range<Rng0> AND
                        viewable_range<Rng1>
                            AND input_range<Rng1>) concat_view<all_t<Rng0>, all_t<Rng1>>
                operator()(Rng0 && rng0, Rng1 && rng1) const
                {
                    return concat_view<all_t<Rng0>, all_t<Rng1>>{
                        all(static_cast<Rng0 &&>(rng0)), all(static_cast<Rng1 &&>(rng1))};
                }
                template(typename Rng0, typename Rng1, typename Rng2)(
                    requires viewable_range<Rng0> AND input_range<Rng0> AND
                        viewable_range<Rng1>
                            AND input_range<Rng1>
                                AND viewable_range<Rng2>
                                    AND input_range<Rng2>)
            ...
            /// \relates concat_fn
            /// \ingroup group-views
            EARANGES_INLINE_VARIABLE(concat_fn, concat)
        } // namespace views
        /// @}
    } // namespace ranges
} // namespace eastl
...
EARANGES_SATISFY_BOOST_RANGE(::ranges::concat_view)
