...
#ifndef EARANGES_VIEW_SAMPLE_HPP
#define EARANGES_VIEW_SAMPLE_HPP
...
namespace ranges
{
    ...
    namespace detail
    {
        template<typename Rng, bool = (bool)sized_sentinel_for<sentinel_t<Rng>, iterator_t<Rng>>>
        class size_tracker
        {
    ...
    class sample_view : public view_facade<sample_view<Rng, URNG>, finite>
    {
        ...
        class cursor
        {
            ...
            EARANGES_NO_UNIQUE_ADDRESS detail::size_tracker<Base> size_;
            ...
            void advance()
            {
                if(parent_->size_ > 0)
                {
                    ...
                    {
                        EARANGES_ASSERT(current_ != ranges::end(parent_->rng_));
                        auto n = pop_size();
                        EARANGES_EXPECT(n > 0);
        ...
        public:
            ...
            cursor(cursor<Other> that)
              ...
              , current_(eastl::move(that.current_))
              , size_(that.size_)
            {}
            ...
            {
                EARANGES_EXPECT(parent_);
                return parent_->size_ <= 0;
            }
            void next()
            {
                EARANGES_EXPECT(parent_);
                EARANGES_EXPECT(parent_->size_ > 0);
                --parent_->size_;
                EARANGES_ASSERT(current_ != ranges::end(parent_->rng_));
                ++current_;
                size_.decrement();
    ...
    public:
        ...
        explicit sample_view(Rng rng, D sample_size, URNG & generator)
          : rng_(eastl::move(rng))
          , size_(sample_size)
          , engine_(eastl::addressof(generator))
        {
            EARANGES_EXPECT(sample_size >= 0);
        }
    ...
    namespace views
    {
        ...
        struct sample_fn : sample_base_fn
        {
            ...
            template(typename Size, typename URNG = detail::default_random_engine)(requires integral<Size> AND uniform_random_bit_generator<URNG>)
            constexpr auto operator()(
                Size n,
                URNG & urng = detail::get_random_engine()) const //
            {
                return make_view_closure(bind_back(sample_base_fn{}, n, detail::reference_wrapper_<URNG>(urng)));
            }
        };

        /// \relates sample_fn
        /// \ingroup group-views
        EARANGES_INLINE_VARIABLE(sample_fn, sample)
    } // namespace views
    /// @}
} // namespace ranges
...
EARANGES_SATISFY_BOOST_RANGE(::ranges::sample_view)
