...
#ifndef EARANGES_VIEW_SPAN_HPP
#define EARANGES_VIEW_SPAN_HPP
...
namespace ranges
{
    ...
    namespace detail
    {
        template(typename To, typename From)(requires integral<To> AND integral<From>)
        constexpr To narrow_cast(From from) noexcept
        {
            using C = common_type_t<To, From>;
            return EARANGES_EXPECT((from > 0) == (static_cast<To>(from) > 0)),
                   EARANGES_EXPECT(static_cast<C>(from) == static_cast<C>(static_cast<To>(from))),
                   static_cast<To>(from);
        }
        ...
        struct span_extent
        {
            ...
            constexpr span_extent(span_index_t size) noexcept
              ...
              : span_extent{(EARANGES_EXPECT(size == N), tag{})}
            {}
        ...
        {
            span_extent() = default;
            constexpr span_extent(span_index_t size) noexcept : size_{((void)EARANGES_EXPECT(size >= 0), size)}
            {}
            constexpr span_index_t size() const noexcept
            {
                return size_;
        ...
        constexpr span_index_t subspan_extent(span_index_t Extent, span_index_t Offset, span_index_t Count) noexcept
        {
            return Count == dynamic_extent && Extent != dynamic_extent ? Extent - Offset : Count;
        }
    } // namespace detail
      ...
      , public detail::span_extent<N>
    {
        ...
        constexpr span(pointer ptr, index_type cnt) noexcept
          : detail::span_extent<N>{(EARANGES_EXPECT(cnt >= 0), cnt)}
          , data_{(EARANGES_EXPECT(0 == cnt || ptr != nullptr), ptr)}
        {}
        ...
        constexpr span<T, Count> first() const noexcept
        {
            ...
            static_assert( N == dynamic_extent || Count <= N, "Count of elements to extract must be less than the static span extent.");
            return EARANGES_EXPECT(Count <= size()),
                   EARANGES_EXPECT(Count == 0 || data_ != nullptr),
                   span<T, Count>{data_, Count};
        }
        constexpr span<T> first(index_type cnt) const noexcept
        {
            return EARANGES_EXPECT(cnt >= 0 && cnt <= size()),
                   EARANGES_EXPECT(cnt == 0 || data_ != nullptr), span<T>{data_, cnt};
        }
        ...
        constexpr span<T, Count> last() const noexcept
        {
            ...
            static_assert(N == dynamic_extent || Count <= N, "Count of elements to extract must be less than the static span extent.");
            return EARANGES_EXPECT(Count <= size()),
                   EARANGES_EXPECT((Count == 0 && size() == 0) || data_ != nullptr),
                   span<T, Count>{data_ + size() - Count, Count};
        }
        constexpr span<T> last(index_type cnt) const noexcept
        {
            return EARANGES_EXPECT(cnt >= 0 && cnt <= size()),
                   EARANGES_EXPECT((cnt == 0 && size() == 0) || data_ != nullptr),
        ...
        constexpr span<T, detail::subspan_extent(N, Offset, Count)> subspan() const
            noexcept
        {
            static_assert(Offset >= 0, "Offset of first element to extract cannot be negative.");
            static_assert(Count >= dynamic_extent, "Count of elements to extract cannot be negative.");
            static_assert(
                N == dynamic_extent ||
                    N >= Offset + (Count == dynamic_extent ? 0 : Count),
                "Sequence of elements to extract must be within the static span extent.");
            return EARANGES_EXPECT(size() >=
                                 Offset + (Count == dynamic_extent ? 0 : Count)),
                   EARANGES_EXPECT((Offset == 0 && Count <= 0) || data_ != nullptr),
            ...
            noexcept
        {
            ...
            static_assert(N == dynamic_extent || N >= Offset,
                          ...
                          "span extent.");
            return EARANGES_EXPECT(size() >= Offset),
                   EARANGES_EXPECT((Offset == 0 && size() == 0) || data_ != nullptr),
                   span < T,
                   N >= Offset ? N - Offset
        ...
        {
            return EARANGES_EXPECT(offset >= 0), EARANGES_EXPECT(size() >= offset),
                   EARANGES_EXPECT((offset == 0 && size() == 0) || data_ != nullptr),
        ...
        constexpr span<T, dynamic_extent> subspan(index_type offset, index_type cnt) const
            noexcept
        {
            return EARANGES_EXPECT(offset >= 0), EARANGES_EXPECT(cnt >= 0),
                   EARANGES_EXPECT(size() >= offset + cnt),
                   EARANGES_EXPECT((offset == 0 && cnt == 0) || data_ != nullptr),
        ...
        {
            return EARANGES_EXPECT(idx >= 0), EARANGES_EXPECT(idx < size()),
                   EARANGES_EXPECT(data_), data_[idx];
        }

        constexpr iterator begin() const noexcept
        {
            return EARANGES_EXPECT(!size() || data_), data_;
        }
        constexpr iterator end() const noexcept
        {
            return EARANGES_EXPECT(!size() || data_), data_ + size();
        }
        ...
        {
            EARANGES_EXPECT(!size() || data());
            EARANGES_EXPECT(!that.size() || that.data());
    ...
    EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<span<T, N>> = true;
    ...
    constexpr span<ElementType> make_span(ElementType * ptr, detail::span_index_t cnt) noexcept
    {
