        ...
        namespace detail
        {
                ...
                narrow_cast(From from) noexcept
            {
                ...
                return EARANGES_EXPECT((from > 0) == (static_cast<To>(from) > 0)),
                       EARANGES_EXPECT(static_cast<C>(from) ==
            ...
            struct span_extent<dynamic_extent>
            {
                span_extent() = default;
                constexpr span_extent(span_index_t size) noexcept
                  : size_{((void)EARANGES_EXPECT(size >= 0), size)}
                {}
                constexpr span_index_t size() const noexcept
                {
                    return size_;
            ...
            constexpr span_index_t subspan_extent(span_index_t Extent,
                                                  span_index_t Offset,
    ...
    CPP_concept span_static_conversion =
          ...
          , public detail::span_extent<N>
        {
            ...
            template(typename Rng)(
                requires(!same_as<span, uncvref_t<Rng>>) AND span_compatible_range<Rng, T>
                    AND span_dynamic_conversion<
                        Rng,
                        N>) constexpr span(Rng &&
                                               rng) noexcept(noexcept(ranges::data(rng),
                                                                      ranges::size(rng)))
              : span{ranges::data(rng),
                     detail::narrow_cast<index_type>(ranges::size(rng))}
            {}

            template(typename Rng)(
                requires(!same_as<span, uncvref_t<Rng>>) AND span_compatible_range<Rng, T>
                    AND span_static_conversion<
                        Rng,
                        N>) constexpr span(Rng &&
                                               rng) noexcept(noexcept(ranges::data(rng)))
            ...
            constexpr span<T, Count> first() const noexcept
            {
                ...
                static_assert(N == dynamic_extent || Count <= N,
                              "Count of elements to extract must be less than the static "
                              "span extent.");
                ...
                const noexcept
            {
                ...
                static_assert(N == dynamic_extent ||
                              ...
                              "Sequence of elements to extract must be within the static "
                              "span extent.");
            ...
            template(typename U,
                     index_type M)(requires equality_comparable_with<T, U>) bool
        ...
        template<typename T, detail::span_index_t N>
        span<unsigned char, detail::byte_size<T>(N)> as_writeable_bytes(
            span<T, N> s) noexcept
        {
