...
#ifndef EARANGES_VIEW_EXCLUSIVE_SCAN_HPP
#define EARANGES_VIEW_EXCLUSIVE_SCAN_HPP
...
namespace ranges
{
    ...
    private:
        ...
        private:
            ...
            // clang-format off
            auto CPP_auto_fun(move_or_copy_init)(eastl::false_type)
            (
                return (rng_->init_)
            )

            // If the base range is single-pass, we can move the init value.
            auto CPP_auto_fun(move_or_copy_init)(eastl::true_type)
            (
                return eastl::move(rng_->init_)
            )
                // clang-format on
            ...
            {
                EARANGES_EXPECT(it != ranges::end(rng_->base()));
                sum_ = invoke(rng_->fun_, static_cast<T &&>(eastl::move(sum_)), *it);
                ++it;
            }
            void prev() = delete;
        };
        ...
        auto begin_adaptor() const //
            -> CPP_ret(adaptor<true>)(requires exclusive_scan_constraints<Rng const, T, Fun const>)
        {
            return {this};
        }
        CPP_member
        auto end_adaptor() const
            -> CPP_ret(meta::if_<use_sentinel_t, adaptor_base, adaptor<true>>)(requires exclusive_scan_constraints<Rng const, T, Fun const>)
        {
            return {this};
        }
    ...
    public:
        ...
        constexpr exclusive_scan_view(Rng rng, T init, Fun fun)
          : exclusive_scan_view::view_adaptor{eastl::move(rng)}
          , init_(eastl::move(init))
          , fun_(eastl::move(fun))
        {}
        CPP_auto_member
    ...
    namespace views
    {
        ...
        {
            template(typename Rng, typename T, typename Fun = plus)(requires exclusive_scan_constraints<Rng, T, Fun>)
            constexpr exclusive_scan_view<all_t<Rng>, T, Fun> //
            operator()(Rng && rng, T init, Fun fun = Fun{}) const
            {
                return {all(static_cast<Rng &&>(rng)), eastl::move(init), eastl::move(fun)};
            }
        };
        ...
        struct exclusive_scan_fn : exclusive_scan_base_fn
        {
            ...
            {
                return make_view_closure(bind_back(exclusive_scan_base_fn{}, eastl::move(init), eastl::move(fun)));
            }
        };

        /// \relates exclusive_scan_fn
        /// \ingroup group-views
        EARANGES_INLINE_VARIABLE(exclusive_scan_fn, exclusive_scan)
    } // namespace views
    /// @}
} // namespace ranges
...
#endif // EARANGES_VIEW_EXCLUSIVE_SCAN_HPP