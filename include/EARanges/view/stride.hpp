...
#ifndef EARANGES_VIEW_STRIDE_HPP
#define EARANGES_VIEW_STRIDE_HPP
...
namespace ranges
{
    ...
    namespace detail
    {
        ...
        struct stride_view_base_ : stride_view_adaptor<Rng>
        {
            ...
            constexpr stride_view_base_(Rng && rng, range_difference_t<Rng> const stride)
              : stride_view_adaptor<Rng>{eastl::move(rng)}
              , stride_{(EARANGES_EXPECT(0 < stride), stride)}
        ...
        protected:
            ...
            {
                EARANGES_EXPECT(0 <= delta && delta < stride_);
                if(0 > offset_)
                    offset_ = delta;
                else
                    EARANGES_EXPECT(offset_ == delta);
            }
            ...
            {
                EARANGES_EXPECT(!check || 0 <= offset_);
                return offset_;
            }
        ...
        private:
            constexpr range_difference_t<Rng> calc_offset(eastl::true_type)
            {
            ...
            constexpr range_difference_t<Rng> calc_offset(eastl::false_type) const noexcept
            {
                return -1;
            }
        };
    ...
    private:
        ...
        public:
            ...
            {
                EARANGES_EXPECT(it != ranges::begin(rng_->base()));
                auto delta = -rng_->stride_;
                if(it == ranges::end(rng_->base()))
                {
                    EARANGES_EXPECT(rng_->get_offset() >= 0);
            ...
            template(typename Other)(
                requires sized_sentinel_for<
                    Other,
                    iterator_t<
                        CRng>>) constexpr range_difference_t<Rng> distance_to(iterator_t<CRng> const &
                                                                                  here,
                                                                              Other const &
                                                                                  there)
                const
    ...
    public:
        ...
        constexpr stride_view(Rng rng, range_difference_t<Rng> const stride)
          : detail::stride_view_base<Rng>{eastl::move(rng), stride}
        {}
        CPP_auto_member constexpr auto CPP_fun(size)()(requires sized_range<Rng>)
        {
            ...
            return (n + static_cast<size_type>(this->stride_) - 1) / static_cast<size_type>(this->stride_);
        }
    ...
    namespace views
    {
        ...
        {
            template(typename Rng)(requires viewable_range<Rng> AND input_range<Rng>) constexpr stride_view<all_t<Rng>> //
        ...
        struct stride_fn : stride_base_fn
        {
            ...
            template(typename Difference)(requires detail::integer_like_<Difference>) constexpr auto
            operator()(Difference step) const
            {
