...
namespace eastl
{
    namespace ranges
    {
        ...
        private:
            ...
            using cache_t =
                detail::non_propagating_cache<iterator_t<Rng>, cycled_view<Rng>,
                                              !common_range<Rng>>;
            ...
            (
                ...
                auto equal(cursor const & pos) const //
                    -> CPP_ret(bool)(requires equality_comparable<iterator>)
                {
                        ...
                        detail::integer_like_<Diff>) void advance(Diff n)
                {
                    ...
                    auto const last = this->get_end_(
                        meta::bool_<(bool)common_range<CRng>>{}, meta::bool_<true>());
                    ...
                    requires sized_sentinel_for<iterator, iterator>)
                {
                    ...
                    auto const first = ranges::begin(rng_->rng_);
                    auto const last = this->get_end_(
                        meta::bool_<(bool)common_range<Rng>>{}, meta::bool_<true>());
                    auto const dist = last - first;
            ...
            CPP_member
            auto begin_cursor() //
                -> CPP_ret(cursor<false>)(requires(!simple_view<Rng>() ||
                                                   !common_range<Rng const>))
            {
                return {this};
            }
            CPP_member
            auto begin_cursor() const //
                -> CPP_ret(cursor<true>)(requires common_range<Rng const>)
            {
                return {this};
            }
        ...
        namespace views
        {
            ...
            struct cycle_fn
            {
                ...
                template(typename Rng)(requires viewable_range<Rng> AND
                                           forward_range<Rng>) cycled_view<all_t<Rng>>
