...
//TODO: BIG PROBLEM TO SOLVE HERE !!!!!!!!!
namespace ranges
{
    /// \cond
    namespace detail
    {
        ...
        struct promote_as_signed_
        {
            ...
            static_assert(sizeof(std::intmax_t) * CHAR_BIT >= N, "Possible extended integral type?");
    ...
    struct EARANGES_EMPTY_BASES closed_iota_view : view_facade<closed_iota_view<From, To>, finite>
    {
    private:
        friend range_access;
        ...
        private:
                ...
                -> CPP_ret(difference_type)(requires detail::advanceable_<From>)
            {
                ...
                return static_cast<D>(detail::iota_distance_(from_, that.from_)) + ((D)that.done_ - (D)done_);
            }
            CPP_member
    ...
    EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<closed_iota_view<From, To>> = true;
    ...
    template<typename From, typename To>
    EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<iota_view<From, To>> = true;
    ...
    namespace views
    {
        struct iota_fn
        {
            template(typename From)( requires weakly_incrementable<From>)
