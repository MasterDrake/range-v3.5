...
//
#ifndef EARANGES_VIEW_CACHE1_HPP
#define EARANGES_VIEW_CACHE1_HPP
...
namespace ranges
{
    ...
    private:
        ...
        auto update_(range_reference_t<Rng> && val) //
            -> CPP_ret(void)( requires assignable_from<range_value_t<Rng> &, range_reference_t<Rng>>)
        {
            if(!cache_)
        ...
        public:
            ...
            range_value_t<Rng> && read() const
            {
                ...
                return eastl::move(*parent_->cache_);
            }
            void next()
            {
                ++current_;
            ...
            auto distance_to(cursor const & that) const //
                -> CPP_ret(difference_type)(requires sized_sentinel_for<iterator_t<Rng>, iterator_t<Rng>>)
            {
                return that.current_ - current_;
            }
            CPP_member
            auto distance_to(sentinel const & that) const //
                -> CPP_ret(difference_type)(requires sized_sentinel_for<sentinel_t<Rng>, iterator_t<Rng>>)
            {
        ...
        cursor end_cursor_impl(eastl::true_type)
        {
            return cursor{this, ranges::end(rng_)};
        }
        sentinel end_cursor_impl(eastl::false_type)
        {
    ...
    public:
        cache1_view() = default;
        constexpr explicit cache1_view(Rng rng) : rng_{eastl::move(rng)}
        {}
        CPP_auto_member
    ...
    namespace views
    {
        struct cache1_fn
        {
            ...
            template(typename Rng)(requires viewable_range<Rng> AND input_range<Rng> AND constructible_from<range_value_t<Rng>, range_reference_t<Rng>>)
        ...
        /// \relates cache1_fn
        /// \ingroup group-views
        EARANGES_INLINE_VARIABLE(view_closure<cache1_fn>, cache1)
    } // namespace views
...
EARANGES_SATISFY_BOOST_RANGE(::ranges::cache1_view)
