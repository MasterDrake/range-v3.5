...
namespace eastl
{
    namespace ranges
    {
            ...
            struct for_each_base_fn
            {
                template(typename Rng, typename Fun)(
                    requires viewable_range<Rng> AND transformable_range<Rng, Fun> AND
                        joinable_range<transform_view<all_t<Rng>, Fun>>) constexpr auto
            ...
            struct for_each_fn : for_each_base_fn
            {
                ...
                constexpr auto operator()(Fun fun) const
                {
                    return make_view_closure(
                        bind_back(for_each_base_fn{}, eastl::move(fun)));
                }
            };
        ...
        struct yield_fn
        {
            template(typename V)(requires copy_constructible<V>) single_view<V>
            operator()(V v) const
            {
        ...
        {
            template(typename Rng)(requires view_<Rng>) Rng operator()(Rng rng) const
            {
                return rng;
            }
        };
        ...
        {
            template(typename F)(requires invocable<F &>) generate_n_view<F> operator()(
                bool b, F f) const
            {
        ...
        template(typename Rng, typename Fun)(
            requires viewable_range<Rng> AND views::transformable_range<Rng, Fun> AND
                input_range<invoke_result_t<Fun &, range_reference_t<Rng>>>) auto
