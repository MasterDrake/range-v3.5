...
#ifndef EARANGES_VIEW_INTERSPERSE_HPP
#define EARANGES_VIEW_INTERSPERSE_HPP
...
namespace ranges
{
    ...
    private:
        ...
        public:
            ...
            constexpr auto equal(iterator_t<CRng> const & it0,
                ...
                -> CPP_ret(bool)(requires sentinel_for<iterator_t<CRng>, iterator_t<CRng>>)
            {
        ...
        constexpr auto begin_adaptor() const //
            -> CPP_ret(cursor_adaptor<true>)(requires range<Rng const>)
        {
            return cursor_adaptor<true>{val_};
        }
        CPP_member
        constexpr auto end_adaptor() //
            -> CPP_ret(cursor_adaptor<false>)(requires common_range<Rng> && (!single_pass_iterator_<iterator_t<Rng>>))
        {
            return cursor_adaptor<false>{val_};
        }
        CPP_member
        constexpr auto end_adaptor() noexcept //
            -> CPP_ret(sentinel_adaptor<false>)(requires (!common_range<Rng>) || single_pass_iterator_<iterator_t<Rng>>)
        {
            return {};
        }
    ...
    template<typename Rng>
    EARANGES_INLINE_VAR constexpr bool enable_borrowed_range<intersperse_view<Rng>> = //
    ...
    namespace views
    {
        struct intersperse_base_fn
        {
            ...
            {
                return {all(static_cast<Rng &&>(rng)), eastl::move(val)};
            }
        };
        ...
        struct intersperse_fn : intersperse_base_fn
        {
            ...
            {
                return make_view_closure(bind_back(intersperse_base_fn{}, eastl::move(t)));
            }
        };

        /// \relates intersperse_fn
        /// \ingroup group-views
        EARANGES_INLINE_VARIABLE(intersperse_fn, intersperse)
    } // namespace views
} // namespace ranges
