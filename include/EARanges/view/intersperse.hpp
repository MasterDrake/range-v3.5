    ...
    namespace ranges
    {
          ...
          : view_adaptor<intersperse_view<Rng>, Rng,
                         (range_cardinality<Rng>::value > 0)
                             ? static_cast<cardinality>(
                                   range_cardinality<Rng>::value * 2 - 1)
        ...
        private:
            ...
            constexpr auto end_adaptor() //
                -> CPP_ret(cursor_adaptor<false>)(
                    ...
                    (!single_pass_iterator_<iterator_t<Rng>>))
            {
                return cursor_adaptor<false>{val_};
            }
            CPP_member
        ...
        namespace views
        {
                ...
                template(typename Rng)(
                    requires viewable_range<Rng> AND input_range<Rng> AND
                        convertible_to<range_reference_t<Rng>, range_value_t<Rng>>
                            AND semiregular<range_value_t<
                                Rng>>) constexpr intersperse_view<all_t<Rng>> //
            ...
            struct intersperse_fn : intersperse_base_fn
            {
                ...
                template(typename T)(requires copyable<T>) constexpr auto operator()(
                    T t) const
                {
                    return make_view_closure(
                        bind_back(intersperse_base_fn{}, eastl::move(t)));
                }
            };
