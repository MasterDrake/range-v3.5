        ...
        EARANGES_FUNC_BEGIN(equal_range)
            ...
            template(typename I,
                ...
                requires forward_iterator<I> AND sentinel_for<S, I> AND
                    indirect_strict_weak_order<C,
                                               V const *,
                                               projected<I, P>>) constexpr subrange<I>
            EARANGES_FUNC(equal_range)(
                I first, S last, V const & val, C pred = C{}, P proj = P{})
            {
                if(EARANGES_CONSTEXPR_IF(sized_sentinel_for<S, I>))
                {
                    ...
                    return aux::equal_range_n(eastl::move(first),
                                              len,
                                              val,
                                              eastl::move(pred),
                                              eastl::move(proj));
                ...
                while(true)
                {
                    ...
                    else if(!invoke(pred, pv, val))
                    {
                        ...
                        // > mid.
                        return {aux::lower_bound_n(eastl::move(first),
                                                   dist,
                                                   val,
                                                   ranges::ref(pred),
                                                   ranges::ref(proj)),
            ...
            template(typename Rng, typename V, typename C = less, typename P = identity)(
                requires forward_range<Rng> AND indirect_strict_weak_order<
                    C,
                    V const *,
                    projected<iterator_t<Rng>, P>>) constexpr borrowed_subrange_t<Rng> //
            EARANGES_FUNC(equal_range)(
                Rng && rng, V const & val, C pred = C{}, P proj = P{}) //
            {
                if(EARANGES_CONSTEXPR_IF(sized_range<Rng>))
                {
                    ...
                    return aux::equal_range_n(
                        begin(rng), len, val, eastl::move(pred), eastl::move(proj));
                }
