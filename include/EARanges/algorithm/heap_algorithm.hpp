    ...
    namespace ranges
    {
        ...
        namespace detail
        {
            struct is_heap_until_n_fn
            {
                ...
                operator()(I const begin_, iter_difference_t<I> const n_, C pred = C{},
                           P proj = P{}) const
                {
            ...
            struct is_heap_n_fn
            {
                template(typename I, typename C = less, typename P = identity)(
                    requires random_access_iterator<I> AND
                        indirect_strict_weak_order<C, projected<I, P>>) constexpr bool
                operator()(I first, iter_difference_t<I> n, C pred = C{},
                           P proj = P{}) const
                {
                    return is_heap_until_n(
                               first, n, eastl::move(pred), eastl::move(proj)) ==
                           first + n;
                }
            };
        ...
        EARANGES_FUNC_BEGIN(is_heap_until)
            ...
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires random_access_iterator<I> AND sentinel_for<S, I> AND
                    indirect_strict_weak_order<C, projected<I, P>>) constexpr I
            EARANGES_FUNC(is_heap_until)(I first, S last, C pred = C{}, P proj = P{})
            {
            ...
            template(typename Rng, typename C = less, typename P = identity)(
                requires random_access_range<Rng> AND indirect_strict_weak_order<
                    C,
        ...
        EARANGES_FUNC_BEGIN(is_heap)
            ...
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires random_access_iterator<I> AND sentinel_for<S, I> AND
                    indirect_strict_weak_order<C, projected<I, P>>) constexpr bool
            EARANGES_FUNC(is_heap)(I first, S last, C pred = C{}, P proj = P{}) //
            {
            ...
            EARANGES_FUNC(is_heap)(Rng && rng, C pred = C{}, P proj = P{}) //
            {
        ...
        namespace detail
        {
            ...
            {
                template<typename I, typename C = less, typename P = identity>
                constexpr void operator()(I first, iter_difference_t<I> len, C pred = C{},
                                          P proj = P{}) const
                {
                    if(len > 1)
                    {
            ...
            {
                template<typename I, typename C = less, typename P = identity>
                constexpr void operator()(I first, iter_difference_t<I> len, I start,
                                          C pred = C{}, P proj = P{}) const
                {
                    ...
                    do
                    {
                        ...
                        *start = iter_move(child_i);
                        ...
                        if((child + 1) < len && invoke(pred,
                                                       invoke(proj, *child_i),
                                                       invoke(proj, *(child_i + 1))))
                        {
                            // right-child exists and is greater than left-child
                            ++child_i;
                            ++child;
        ...
        EARANGES_FUNC_BEGIN(push_heap)
            ...
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires random_access_iterator<I> AND sentinel_for<S, I> AND
                    sortable<I, C, P>) constexpr I
            EARANGES_FUNC(push_heap)(I first, S last, C pred = C{}, P proj = P{})
            {
            ...
            template(typename Rng, typename C = less, typename P = identity)(
                requires random_access_range<Rng> AND
                    sortable<iterator_t<Rng>, C, P>) constexpr borrowed_iterator_t<Rng> //
        ...
        namespace detail
        {
            ...
            {
                template(typename I, typename C = less,
                         typename P = identity)(requires random_access_iterator<I> AND
                                                    sortable<I, C, P>) constexpr void
                operator()(I first, iter_difference_t<I> len, C pred = C{},
                           P proj = P{}) const
                {
                    if(len > 1)
                    {
        ...
        EARANGES_FUNC_BEGIN(pop_heap)
            ...
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires random_access_iterator<I> AND sentinel_for<S, I> AND
                    sortable<I, C, P>) constexpr I
            EARANGES_FUNC(pop_heap)(I first, S last, C pred = C{}, P proj = P{})
            {
                auto n = distance(first, last);
        ...
        EARANGES_FUNC_BEGIN(sort_heap)
            ...
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires random_access_iterator<I> AND sentinel_for<S, I> AND
                    sortable<I, C, P>) constexpr I
            EARANGES_FUNC(sort_heap)(I first, S last, C pred = C{}, P proj = P{})
            {
                iter_difference_t<I> const n = distance(first, last);
                for(auto i = n; i > 1; --i)
