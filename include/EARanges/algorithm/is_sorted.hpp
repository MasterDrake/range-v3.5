        ...
        EARANGES_FUNC_BEGIN(is_sorted)
            ...
            template(typename I, typename S, typename R = less, typename P = identity)(
                requires forward_iterator<I> AND sentinel_for<S, I> AND
                    indirect_strict_weak_order<R, projected<I, P>>) constexpr bool
            EARANGES_FUNC(is_sorted)(I first, S last, R rel = R{}, P proj = P{})
            {
                return is_sorted_until(eastl::move(first),
                                       last,
                                       eastl::move(rel),
                                       eastl::move(proj)) == last;
            }
            ...
            EARANGES_FUNC(is_sorted)(Rng && rng, R rel = R{}, P proj = P{}) //
            {
