...
#ifndef EARANGES_ALGORITHM_NTH_ELEMENT_HPP
#define EARANGES_ALGORITHM_NTH_ELEMENT_HPP
...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            template(typename I, typename C, typename P)(
                requires forward_iterator<I> AND
                    indirect_relation<C, projected<I, P>>) unsigned sort3(I x, I y, I z,
                                                                          C & pred,
                                                                          P & proj)
            {
                unsigned r = 0;
            ...
            template(typename I, typename C, typename P)(
                requires bidirectional_iterator<I> AND
                    indirect_relation<C, projected<I, P>>) void selection_sort(I first,
                                                                               I last,
                                                                               C & pred,
                                                                               P & proj)
            {
                ...
                {
                    I i = ranges::min_element(
                        first, last, eastl::ref(pred), eastl::ref(proj));
                    if(i != first)
        ...
        /// @{
        EARANGES_FUNC_BEGIN(nth_element)

            /// \brief function template \c nth_element
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires random_access_iterator<I> AND sortable<I, C, P>) constexpr I
            EARANGES_FUNC(nth_element)(
                I first, I nth, S end_, C pred = C{}, P proj = P{}) //
            {
                ...
                while(true)
                {
                    ...
                    if(!invoke(
                           pred, invoke(proj, *i), invoke(proj, *m))) // if *first == *m
                    {
                        ...
                        while(true)
                        {
                            if(i == --j)
                            {
                                ...
                                // Parition instead into [first, i) == *first and *first <
                                // [i, last)
                                ++i; // first + 1
                                j = last;
                                if(!invoke(pred,
                                           invoke(proj, *first),
                                           invoke(proj,
                                                  *--j))) // we need a guard if *first ==
                                                          // *(last-1)
                                {
                                    while(true)
                                    {
                                        if(i == j)
                                            return end_orig; // [first, last) all
                                                             // equivalent elements
                                        if(invoke(pred,
                                                  invoke(proj, *first),
                                                  invoke(proj, *i)))
                                ...
                                // [first, i) == *first and *first < [j, last) and j ==
                                // last -
                                // 1
                                if(i == j)
                                    return end_orig;
