        ...
        EARANGES_FUNC_BEGIN(minmax)
            ...
            template(typename T, typename C = less, typename P = identity)(
                requires indirect_strict_weak_order<
                    C,
                    projected<T const *, P>>) constexpr minmax_result<T const &>
            EARANGES_FUNC(minmax)(T const & a, T const & b, C pred = C{}, P proj = P{}) //
            {
                ...
                requires input_range<Rng> AND
                    indirect_strict_weak_order<C, projected<iterator_t<Rng>, P>>
                        AND indirectly_copyable_storable<
                            iterator_t<Rng>,
                            range_value_t<
                                Rng> *>) constexpr minmax_result<range_value_t<Rng>> //
                    ...
                    while(++first != last)
                    {
                        ...
                        else
                        {
                            if(invoke(pred, invoke(proj, tmp1), invoke(proj, result.min)))
                                result.min = eastl::move(tmp1);
                            if(!invoke(
                                   pred, invoke(proj, tmp2), invoke(proj, result.max)))
            ...
            template(typename T, typename C = less, typename P = identity)(
                requires copyable<T> AND indirect_strict_weak_order<
                    C,
                    projected<T const *, P>>) constexpr minmax_result<T>
            EARANGES_FUNC(minmax)(
