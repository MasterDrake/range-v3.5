...
namespace ranges
{
    ...
    namespace detail
    {
        ...
        constexpr subrange<I> rotate_(I first, I middle, S last, eastl::forward_iterator_tag)
        {
            return detail::rotate_forward(first, middle, last);
        }

        template<typename I>
        constexpr subrange<I> rotate_(I first, I middle, I last, eastl::forward_iterator_tag)
        {
        ...
        template<typename I>
        constexpr subrange<I> rotate_(I first, I middle, I last, eastl::bidirectional_iterator_tag)
        {
            using value_type = iter_value_t<I>;
            if(detail::is_trivially_move_assignable_v<value_type>)
            {
        ...
        template<typename I>
        constexpr subrange<I> rotate_(I first, I middle, I last, eastl::random_access_iterator_tag)
        {
            using value_type = iter_value_t<I>;
            if(detail::is_trivially_move_assignable_v<value_type>)
            {
    ...
    EARANGES_FUNC_BEGIN(rotate)
        ...
        template(typename I, typename S)(requires permutable<I> AND sentinel_for<S, I>)
        constexpr subrange<I> EARANGES_FUNC(rotate)(I first, I middle, S last) //
        {
            if(first == middle)
            {
        ...
        /// \overload
        template(typename Rng, typename I = iterator_t<Rng>)(requires range<Rng> AND permutable<I>)
        constexpr borrowed_subrange_t<Rng> EARANGES_FUNC(rotate)(Rng && rng, I middle) //
        {
