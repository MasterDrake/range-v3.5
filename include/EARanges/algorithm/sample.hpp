...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            sample_result<I, O> sample_sized_impl(I first, S last,
                                                  iter_difference_t<I> pop_size, O out,
        ...
        EARANGES_FUNC_BEGIN(sample)
                ...
                requires input_iterator<I> AND sentinel_for<S, I> AND
                    weakly_incrementable<O>
                        AND indirectly_copyable<I, O>
                            AND uniform_random_bit_generator<
                                eastl::remove_reference_t<Gen>>
                                AND(random_access_iterator<O> || forward_iterator<I> ||
                                    sized_sentinel_for<S, I>)) sample_result<I, O>
            EARANGES_FUNC(sample)(I first,
                                  S last,
                                  O out,
                                  iter_difference_t<O> const n,
                                  Gen && gen = detail::get_random_engine())
            {
                if(EARANGES_CONSTEXPR_IF(forward_iterator<I> ||
            ...
            template(typename I,
                ...
                requires input_iterator<I> AND sentinel_for<S, I> AND
                    weakly_incrementable<iterator_t<ORng>>
                        AND indirectly_copyable<I, iterator_t<ORng>>
                            AND uniform_random_bit_generator<
                                eastl::remove_reference_t<Gen>>
                                AND(forward_range<ORng> || sized_range<ORng>)
                                    AND(random_access_iterator<iterator_t<ORng>> ||
            ...
            /// \overload
            template(
                typename Rng, typename O, typename Gen = detail::default_random_engine &)(
                requires input_range<Rng> AND weakly_incrementable<O> AND
                    indirectly_copyable<iterator_t<Rng>, O>
                        AND uniform_random_bit_generator<eastl::remove_reference_t<Gen>>
                            AND(random_access_iterator<O> || forward_range<Rng> ||
                                sized_range<Rng>))
                sample_result<borrowed_iterator_t<Rng>, O>
                EARANGES_FUNC(sample)(Rng && rng,
                                      O out,
                                      iter_difference_t<O> const n,
                                      Gen && gen = detail::get_random_engine()) //
            {
            ...
            template(typename IRng,
                ...
                requires input_range<IRng> AND range<ORng> AND
                    indirectly_copyable<iterator_t<IRng>, iterator_t<ORng>>
                        AND uniform_random_bit_generator<eastl::remove_reference_t<Gen>>
                            AND(random_access_iterator<iterator_t<ORng>> ||
                                forward_range<IRng> || sized_range<IRng>)
                                AND(forward_range<ORng> || sized_range<ORng>))
                sample_result<borrowed_iterator_t<IRng>, borrowed_iterator_t<ORng>> //
                EARANGES_FUNC(sample)(
                    IRng && rng, ORng && out, Gen && gen = detail::get_random_engine())
            {
