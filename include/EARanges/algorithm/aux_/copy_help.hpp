...
#ifndef EARANGES_INTERNAL_COPY_HELP_H
#define EARANGES_INTERNAL_COPY_HELP_H
...
//TODO: We need to know where eastl::copy and move(not the cast) are called so they can use this instead...
...
namespace ranges
{
	...
	struct move_and_copy_helper
	{
		...
		constexpr static OutputIterator move_or_copy(InputIterator first, InputSentinel last, OutputIterator result)
		{
	...
	struct move_and_copy_helper<EASTL_ITC_NS::random_access_iterator_tag, false, false>
	{
		...
		constexpr static OutputIterator move_or_copy(InputIterator first, InputSentinel last, OutputIterator result)
		{
			...
			//HACKHACKHACK: Not sure how good this specialization would be using ranges::distance()... but it's constexpr so who knows(not me), maybe it's fine.
			for(difference_type n = ranges::distance(first, last); n > 0; --n, ++first, ++result)
				*result = *first;
	...
	template <bool isMove, typename InputIterator,typename InputSentinel, typename OutputIterator>
	constexpr inline OutputIterator move_and_copy_chooser(InputIterator first, InputSentinel last, OutputIterator result)
	{
	...
	template <bool isMove, typename InputIterator, typename InputSentinel, typename OutputIterator>
	constexpr inline OutputIterator move_and_copy_unwrapper(InputIterator first, InputSentinel last, OutputIterator result)
	{
		return OutputIterator(move_and_copy_chooser<isMove>(eastl::unwrap_iterator(first), eastl::unwrap_iterator(last), eastl::unwrap_iterator(result))); // Have to convert to OutputIterator because unwrap_iterator(result) could be a T*
	}
	...
	template <typename InputIterator, typename InputSentinel, typename OutputIterator>
	constexpr inline OutputIterator move(InputIterator first, InputSentinel last, OutputIterator result)
	{
		return move_and_copy_unwrapper<true>(eastl::unwrap_iterator(first), eastl::unwrap_iterator(last), result);
	}
	...
	/// may in fact be within the input range.
	///
	/// Complexity: Exactly 'last - first' assignments.
	///
	template <typename InputIterator, typename InputSentinel, typename OutputIterator>
	constexpr inline OutputIterator copy(InputIterator first, InputSentinel last, OutputIterator result)
	{
		const bool isMove = eastl::is_move_iterator<InputIterator>::value; EA_UNUSED(isMove);

		return move_and_copy_unwrapper<isMove>(eastl::unwrap_iterator(first), eastl::unwrap_iterator(last), result);
	}
	}
} // namespace ranges

#endif // EARANGES_INTERNAL_COPY_HELP_H
