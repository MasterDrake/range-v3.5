...
#ifndef EARANGES_INTERNAL_FILL_HELP_H
#define EARANGES_INTERNAL_FILL_HELP_H
...
//TODO: Check that the other part of earanges uses this instead of eastl::fill, but I don't think so, just move and copy.
namespace ranges
{
	namespace detail
	{
	...
	struct fill_imp
	{
        ...
        constexpr static ForwardIterator do_fill(ForwardIterator first, ForwardSentinel last, const T & value)
		{
	...
	struct fill_imp<true>
	{
        ...
        constexpr static ForwardIterator do_fill(ForwardIterator first, ForwardSentinel last, const T & value)
		{
			using value_type = eastl::iterator_traits<ForwardIterator>::value_type;
	...
	///
    template<typename ForwardIterator, typename ForwardSentinel, typename T>
    constexpr inline ForwardIterator fill(ForwardIterator first, ForwardSentinel last, const T & value)
	{
		return fill_imp<eastl::is_scalar<T>::value>::do_fill(first, last, value);
		...
		//const bool bEasyCopy = eastl::type_and<eastl::has_trivial_constructor<T>::value, eastl::integral_constant<bool, (sizeof(T) <= 16)>::value;
		//eastl::fill_imp<bEasyCopy>::do_fill(first, last, value);
	} 
	
	//TODO: Check in linux with gcc and clang...
	...
	inline char* fill(char* first, char* last, const char& c) // It's debatable whether we should use 'char& c' or 'char c' here.
	{
	...
	struct fill_n_imp
	{
		...
		constexpr static OutputIterator do_fill(OutputIterator first, Size n, const T& value)
		{
			for(; n-- > 0; ++first)
				*first = value;
			return first;
	...
	{
		return fill_n_imp<eastl::is_scalar<T>::value>::do_fill(first, n, value);
	}
