...
//
#ifndef EARANGES_ALGORITHM_FIND_END_HPP
#define EARANGES_ALGORITHM_FIND_END_HPP
    ...
    namespace ranges
    {
        ...
        namespace detail
        {
            template(typename I, typename S)(
                requires input_iterator<I> AND sentinel_for<S, I>) constexpr I
                next_to_if(I i, S s, eastl::true_type)
            {
                return ranges::next(i, s);
            }

            template(typename I, typename S)(
                requires input_iterator<I> AND sentinel_for<S, I>) constexpr S
                next_to_if(I, S s, eastl::false_type)
            {
                return s;
            }
            ...
            template(bool B, typename I, typename S)(
                requires input_iterator<I> AND
                    sentinel_for<S, I>) constexpr meta::if_c<B, I, S> next_to_if(I i, S s)
            {
                return detail::next_to_if(
                    eastl::move(i), eastl::move(s), meta::bool_<B>{});
            }

            template<typename I1, typename S1, typename I2, typename S2, typename R,
                     typename P>
            constexpr subrange<I1> find_end_impl(I1 begin1, S1 end1, I2 begin2, S2 end2,
                                                 R pred, P proj,
            ...
            constexpr subrange<I1> find_end_impl(I1 begin1, I1 end1, I2 begin2, I2 end2,
                                                 R pred, P proj,
                                                 ...
                                                 eastl::bidirectional_iterator_tag)
            {
                ...
                while(true)
                {
                    // Find end element in sequence 1 that matches *(end2-1), with a
                                                 ...
                                                 eastl::random_access_iterator_tag)
            {
                ...
                while(true)
                {
                    ...
                    do
                        if(m2 == begin2)
                            return {m1, ++l1};
                    // no need to check range on m1 because s guarantees we have enough
                    // source
                    while(invoke(pred, invoke(proj, *--m1), *--m2));
                }
            }
        ...
        EARANGES_FUNC_BEGIN(find_end)
                    ...
                    forward_iterator<I2>
                        AND sentinel_for<S2, I2>
                            AND indirect_relation<R,
                                                  projected<I1, P>,
                                                  I2>) constexpr subrange<I1>
            EARANGES_FUNC(find_end)(
            ...
            template(typename Rng1,
                ...
                requires forward_range<Rng1> AND forward_range<Rng2> AND
                    indirect_relation<
                        R,
                        projected<iterator_t<Rng1>, P>,
                        iterator_t<Rng2>>) constexpr borrowed_subrange_t<Rng1>
            EARANGES_FUNC(find_end)(
