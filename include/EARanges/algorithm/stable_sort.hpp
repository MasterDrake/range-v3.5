...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            // buffer points to raw memory, we create objects, and then restore the buffer
            // to raw memory by destroying the objects on return.
            ...
            template<typename I, typename V, typename C, typename P>
            void stable_sort_adaptive(I first, I last, V * buffer,
        ...
        EARANGES_FUNC_BEGIN(stable_sort)
            ...
            template(typename I, typename S, typename C = less, typename P = identity)(
                requires sortable<I, C, P> AND random_access_iterator<I> AND
                    sentinel_for<S, I>) I
            EARANGES_FUNC(stable_sort)(I first, S end_, C pred = C{}, P proj = P{})
            {
                I last = ranges::next(first, end_);
                using D = iter_difference_t<I>;
                using V = iter_value_t<I>;
                D len = last - first;
                auto buf = len > 256 ? detail::get_temporary_buffer<V>(len)
                                     : detail::value_init{};
