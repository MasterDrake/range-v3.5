        ...
        namespace detail
        {
                                    ...
                                    eastl::forward_iterator_tag fi)
            {
                ...
                { // The buffer is big enough to use
                    // Move the falses into the temporary buffer, and the trues to the
                    // front of the line Update first to always point to the last of the
                    // trues
                    ...
                    *buf = iter_move(first);
                    ...
                    // Move falses back into range, but don't mess up first which points
                    // to first false
                ...
                // TTTFFFFF??????????
                // f  ff   m         l
                // recurse on [middle, last], except increase middle until *(middle) is
                // false, *last know to be true
                I m1 = middle;
                D len_half = len - half;
                                    ...
                                    eastl::forward_iterator_tag fi)
            {
                ...
                difference_type const alloc_limit = 3; // might want to make this a
                                                       // function of trivial assignment.
                ...
                auto p = len_end.first >= alloc_limit
                             ...
                             : detail::value_init{};
                eastl::unique_ptr<value_type, detail::return_temporary_buffer> const h{
                    p.first};
            ...
            I stable_partition_impl(I first, I last, C pred, P proj, D len, Pair p,
                                    eastl::bidirectional_iterator_tag bi)
            {
                // *first is known to be false
                ...
                D half = len / 2; // half >= 2
                advance(middle, half);
                // recurse on [first, middle-1], except reduce middle-1 until *(middle-1)
                // is true, *first know to be false F????????????????T f       m        l
                I m1 = middle;
                I begin_false = first;
                D len_half = half;
                while(!invoke(pred, invoke(proj, *--m1)))
                                    ...
                                    eastl::bidirectional_iterator_tag bi)
            {
                ...
                // Either prove [first, last) is all false and return first, or point last
                // to last true
                ...
                auto p = len >= alloc_limit
                             ? detail::get_temporary_buffer<value_type>(len)
                             : detail::value_init{};
                eastl::unique_ptr<value_type, detail::return_temporary_buffer> const h{
                    p.first};
            ...
            template(typename I, typename S, typename C, typename P = identity)(
                requires bidirectional_iterator<I> AND sentinel_for<S, I> AND
                    indirect_unary_predicate<C, projected<I, P>>
                        AND permutable<I>) I
            EARANGES_FUNC(stable_partition)(I first, S last, C pred, P proj = P{})
            {
            ...
            template(typename Rng, typename C, typename P = identity)(
                requires bidirectional_range<Rng> AND
                    indirect_unary_predicate<C, projected<iterator_t<Rng>, P>>
                        AND permutable<iterator_t<Rng>>) borrowed_iterator_t<Rng> //
