    ...
    namespace ranges
    {
        ...
        public:
            ...
            constexpr auto operator()(F T::*f, T1 && t1, Args &&... args) const
                noexcept(noexcept((invoke_fn::coerce<T>((T1 &&)t1, 0).*
                                   f)((Args &&)args...)))
        ...
        namespace detail
        {
            ...
            struct _invoke_result_<
                meta::void_<decltype(invoke(eastl::declval<Fun>(),
                                            eastl::declval<Args>()...))>,
                Fun, Args...>
            {
                using type =
        ...
        namespace detail
        {
            ...
            struct is_nothrow_invocable_impl_<true>
            {
                ...
                {
                    return noexcept(
                        invoke(eastl::declval<Fn>(), eastl::declval<Args>()...));
                }
            };
        } // namespace detail
        /// \endcond
        ...
        template<typename Fn, typename... Args>
        EARANGES_INLINE_VAR constexpr bool is_invocable_v =
            meta::is_trait<invoke_result<Fn, Args...>>::value;

        template<typename Fn, typename... Args>
        EARANGES_INLINE_VAR constexpr bool is_nothrow_invocable_v =
            detail::is_nothrow_invocable_impl_<
                is_invocable_v<Fn, Args...>>::template apply<Fn, Args...>();
        /// @}
    } // namespace ranges
} // namespace eastl
