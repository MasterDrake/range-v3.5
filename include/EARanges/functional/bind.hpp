...
namespace ranges
{
    ...
    T && bind_forward(meta::_t<eastl::remove_reference<T>> && t) noexcept
    {
        ...
        static_assert(!eastl::is_lvalue_reference<T>::value, "You didn't just do that!");
        return static_cast<T &&>(t);
    }

    template<typename T>
    struct bind_element : meta::if_c<EARANGES_IS_SAME(detail::decay_t<T>, T), meta::id<T>,bind_element<detail::decay_t<T>>>
    {};
    ...
    public:
        protector() = default;
        protector(Bind b) : bind_(eastl::move(b))
        {}
        // clang-format off
