...
#ifndef EARANGES_UTILITY_TAGGED_PAIR_HPP
#define EARANGES_UTILITY_TAGGED_PAIR_HPP
...
namespace ranges
{
    ...
    namespace detail
    {
        ...
        {
            using type = typename First::template getter<Base, I, meta::_t<tagged_chain<Base, I + 1, Rest...>>>;
        };
    } // namespace detail
    ...
    class EARANGES_EMPTY_BASES EARANGES_DEPRECATED(
...
#endif
    {
        ...
        struct can_convert
          : meta::bool_<!EARANGES_IS_SAME(Other, Base) &&
    ...
    public:
        ...
        constexpr tagged(tagged<Other, Tags...> && that) //
            noexcept(eastl::is_nothrow_constructible<Base, Other>::value)
          : base_t(static_cast<Other &&>(that))
        {}
        template(typename Other)(
            requires can_convert<Other>::value)               //
        constexpr tagged(tagged<Other, Tags...> const & that) //
            noexcept(eastl::is_nothrow_constructible<Base, Other const &>::value)
        ...
        constexpr tagged & operator=(tagged<Other, Tags...> && that) //
            noexcept(
                noexcept(eastl::declval<Base &>() = static_cast<Other &&>(that)))
        {
        ...
        constexpr tagged & operator=(tagged<Other, Tags...> const & that) //
            noexcept(
                noexcept(eastl::declval<Base &>() = static_cast<Other const &>(that)))
        {
        ...
        template(typename U)(
            requires (!same_as<tagged, detail::decay_t<U>>) AND
                satisfies<Base &, eastl::is_assignable, U>)
        constexpr tagged & operator=(U && u) //
            noexcept(noexcept(eastl::declval<Base &>() = static_cast<U &&>(u)))
        {
    ...
    using tagged_pair EARANGES_DEPRECATED("ranges::tagged_pair is deprecated.") =
        tagged<eastl::pair<detail::tag_elem<F>, detail::tag_elem<S>>, detail::tag_spec<F>,
               detail::tag_spec<S>>;
    ...
    EARANGES_DEPRECATED("ranges::make_tagged_pair is deprecated.")
    constexpr R make_tagged_pair(T1 && t1, T2 && t2) noexcept(
        eastl::is_nothrow_constructible<R, T1, T2>::value)
    {
        return {static_cast<T1 &&>(t1), static_cast<T2 &&>(t2)};
    }
} // namespace ranges

#define EARANGES_DEFINE_TAG_SPECIFIER(NAME)                                               \
    namespace tag                                                                       \
    {                                                                                   \
        struct NAME                                                                     \
        {                                                                               \
            ...
            public:                                                                     \
                ...
                constexpr meta::_t<eastl::tuple_element<I, Untagged>> & NAME() &          \
                    noexcept(noexcept(detail::adl_get<I>(eastl::declval<Untagged &>())))  \
                {                                                                       \
                ...
                constexpr meta::_t<eastl::tuple_element<I, Untagged>> const & NAME()      \
                    const & noexcept(                                                   \
                        noexcept(detail::adl_get<I>(eastl::declval<Untagged const &>()))) \
                {                                                                       \
...
namespace eastl
{
    ...
    template<size_t N, typename Untagged, typename... Tags>
    struct tuple_element<N, ::ranges::tagged<Untagged, Tags...>> : tuple_element<N, Untagged>
    {};
} // namespace std
