...
#ifndef EARANGES_UTILITY_OPTIONAL_HPP
#define EARANGES_UTILITY_OPTIONAL_HPP

#include <exception>
#include <EASTL/initializer_list.h>
#include <EASTL/memory.h>
//TODO: A cosa serve?? addressof? #include <mnemory>
#include <new>
...
#include <EARanges/utility/in_place.hpp>
...
namespace ranges
{
    ...
    namespace detail
    {
        ...
        {
            template<typename T, bool = eastl::is_trivially_destructible<T>::value>
            struct optional_storage
            {
                union
                {
                    char dummy_;
                    meta::_t<eastl::remove_cv<T>> data_;
                };
                bool engaged_;
            ...
            private:
                struct tag
                {};
                constexpr optional_storage(tag, eastl::false_type) noexcept
                  : dummy_{}
                  , engaged_{false}
                {}
                constexpr optional_storage(tag, eastl::true_type) noexcept
                  : data_{}
                  , engaged_{false}
                {}
            };
            ...
            struct optional_base : private optional_storage<T>
            {
                ...
                {
                    return EARANGES_EXPECT(engaged_), data_;
                }
                ...
                constexpr T && operator*() && noexcept
                {
                    return EARANGES_EXPECT(engaged_), detail::move(data_);
                }
                constexpr T const && operator*() const && noexcept
                {
                    return EARANGES_EXPECT(engaged_), detail::move(data_);
                ...
                constexpr T * operator->() noexcept
                {
                    return EARANGES_EXPECT(engaged_), detail::addressof(data_);
                }
                constexpr T const * operator->() const noexcept
                {
                    return EARANGES_EXPECT(engaged_), detail::addressof(data_);
            ...
            protected:
                ...
                T & construct_from(Args &&... args)
                    noexcept(eastl::is_nothrow_constructible<T, Args...>::value)
                {
                    EARANGES_EXPECT(!engaged_);
                    auto const address = static_cast<void *>(eastl::addressof(data_));
                    ::new(address) T(static_cast<Args &&>(args)...);
                    engaged_ = true;
                    return data_;
                }
                template(typename I)(
                    requires constructible_from<T, decltype(*eastl::declval<const I &>())>)
                ...
                constexpr void assign_from(U && that) noexcept(
                    eastl::is_nothrow_constructible<T, decltype(*static_cast<U &&>(that))>::
                        value && eastl::is_nothrow_assignable<
            ...
            private:
                constexpr void swap_(eastl::true_type, optional_base & that) noexcept
                {
                    ranges::swap(static_cast<optional_storage<T> &>(*this),
                                 static_cast<optional_storage<T> &>(that));
                }
                constexpr void swap_(eastl::false_type, optional_base & that) noexcept(
            ...
            struct optional_base<T &>
            {
                ...
                {
                    return EARANGES_EXPECT(ptr_), *ptr_;
                }
                constexpr T * operator->() const noexcept
                {
                    return EARANGES_EXPECT(ptr_), ptr_;
            ...
            using copy_construct_layer =
                meta::if_c<eastl::is_copy_constructible<T>::value &&
            ...
            struct optional_move : copy_construct_layer<T>
            {
                ...
                {
                    if(that.has_value())
                        this->construct_from(eastl::move(*that));
                }
            ...
            using move_construct_layer =
                meta::if_c<eastl::is_move_constructible<T>::value &&
                               !detail::is_trivially_move_constructible_v<T>,
            ...
            using copy_assign_layer = meta::if_c<
                eastl::is_copy_constructible<T>::value && eastl::is_copy_assignable<T>::value,
                meta::if_c<eastl::is_reference<T>::value ||
                               !(detail::is_trivially_copy_constructible_v<T> &&
            ...
            struct optional_move_assign : copy_assign_layer<T>
            {
                ...
                {
                    this->assign_from(eastl::move(that));
                    return *this;
                }
            ...
            using move_assign_layer = meta::if_c<
                eastl::is_move_constructible<T>::value && eastl::is_move_assignable<T>::value,
                meta::if_c<eastl::is_reference<T>::value ||
                               !(detail::is_trivially_move_constructible_v<T> &&
                                 detail::is_trivially_move_assignable_v<T>),
    ...
    public:
        ...
        static_assert(eastl::is_object<T>::value || eastl::is_lvalue_reference<T>::value, "");
        ...
        constexpr explicit optional(in_place_t, std::initializer_list<E> il,
                                    Args &&... args) //
            noexcept(eastl::is_nothrow_constructible<T, std::initializer_list<E> &,
                                                   Args...>::value)
        ...
        template(typename U = T)(
            requires (!same_as<optional, detail::decay_t<U>>) AND
                (!(satisfies<T, eastl::is_scalar> && same_as<T, detail::decay_t<U>>)) AND
                constructible_from<T, U> AND
                assignable_from<T &, U>)
        constexpr optional & operator=(U && u) noexcept(
            eastl::is_nothrow_constructible<T, U>::value &&
        ...
        {
            base_t::assign_from(eastl::move(that));
            return *this;
        }
        ...
        T & emplace(std::initializer_list<E> il, Args &&... args) noexcept(
            eastl::is_nothrow_constructible<T, std::initializer_list<E> &, Args...>::value)
        {
            reset();
