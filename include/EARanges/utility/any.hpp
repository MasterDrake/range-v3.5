...
#ifndef EARANGES_UTILITY_ANY_HPP
#define EARANGES_UTILITY_ANY_HPP
...
namespace ranges
{
    ...
    template<typename T>
    meta::if_c<eastl::is_reference<T>() || copyable<T>, T> any_cast(any &);

    template<typename T>
    meta::if_c<eastl::is_reference<T>() || copyable<T>, T> any_cast(any const &);
    ...
    private:
        template<typename T>
        friend meta::if_c<eastl::is_reference<T>() || (bool)copyable<T>, T> any_cast(any &);

        template<typename T>
        friend meta::if_c<eastl::is_reference<T>() || (bool)copyable<T>, T> any_cast(any const &);
        ...
        eastl::unique_ptr<_any_::interface> ptr_;

    public:
        any() noexcept = default;
        template(typename TRef,
                 typename T = detail::decay_t<TRef>)(requires(!same_as<T, any>)
                                                         AND copyable<T>) //
            any(TRef && t)
        ...
        template(typename TRef,
                 typename T = detail::decay_t<TRef>)(requires(!same_as<T, any>)AND copyable<T>) //
            any &
            operator=(TRef && t)
        {
    ...
    /// \throw bad_any_cast
    template<typename T>
    meta::if_c<eastl::is_reference<T>() || copyable<T>, T> any_cast(any & x)
    {
    ...
    /// \overload
    template<typename T>
    meta::if_c<eastl::is_reference<T>() || copyable<T>, T> any_cast(any const & x)
    {
        if(x.type() != typeid(detail::decay_t<T>))
            throw bad_any_cast{};
