...
namespace eastl
{
    namespace ranges
    {
        ...
        namespace detail
        {
            ...
            {
                none, // Nothing special: get() returns a reference to a T member
                      // subobject
                ebo,  // Apply Empty Base Optimization: get() returns a reference to a T
                     // base subobject
                coalesce // Coalesce all Ts into one T: get() returns a reference to a
                         // static T singleton
            };
            ...
            using could_be_lambda =
                meta::bool_<!eastl::is_default_constructible<Fn>::value &&
            ...
            template<typename T,
                     typename = meta::if_<meta::strict_and<
                         eastl::is_empty<T>,
            ...
            template<typename T, typename = meta::if_<meta::strict_and<
                                     eastl::is_empty<T>, detail::is_trivial<T>>>>
        ...
        public:
            ...
            template(typename E)(
                requires(!same_as<box, detail::decay_t<E>>) AND constructible_from<
                    Element, E>) constexpr explicit(!convertible_to<E, Element>)
                box(E && e) noexcept(
                    eastl::is_nothrow_constructible<Element, E>::value) //
              : value(static_cast<E &&>(e))
            {}
#else
            template(typename E)(
                requires(
                    !same_as<box, detail::decay_t<E>>) AND constructible_from<Element, E>
                    AND convertible_to<
                        E,
                        Element>) constexpr box(E &&
                                                    e) noexcept(eastl::
            ...
            template(typename E)(
                requires(
                    ...
                    AND(!convertible_to<
                        E,
                        Element>)) constexpr explicit box(E &&
                                                              e) noexcept(eastl::
                                                                              is_nothrow_constructible<
                                                                                  Element,
                                                                                  E>::
...
#else
            template(typename E)(
                requires(!same_as<box, detail::decay_t<E>>)
                    AND constructible_from<Element, E>
                        AND convertible_to<E, Element>) constexpr box(E &&) noexcept
            {}
            template(typename E)(
                requires(!same_as<box, detail::decay_t<E>>)
                    AND constructible_from<Element, E>
                        AND(!convertible_to<
                            E, Element>)) constexpr explicit box(E &&) noexcept
            {}
#endif
