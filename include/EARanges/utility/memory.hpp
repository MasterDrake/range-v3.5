...
namespace eastl
{
    namespace ranges
    {
        ...
        {
            template<typename T>
            eastl::pair<T *, std::ptrdiff_t> get_temporary_buffer_impl(
                std::size_t n) noexcept
            {
                ...
                for(; ptr == nullptr && n > 0; n /= 2)
                {
                    ...
                    if(EARANGES_CONSTEXPR_IF(alignof(T) >
            ...
            eastl::pair<T *, std::ptrdiff_t> get_temporary_buffer(D count) noexcept
            {
                ...
                return detail::get_temporary_buffer_impl<T>(
            ...
            template(typename T,
                     typename... Args)(requires(!eastl::is_array<T>::value)) //
        ...
        public:
            ...
            auto operator=(Val const & val) //
                -> CPP_ret(raw_storage_iterator &)(requires copy_constructible<Val>)
            {
            ...
            auto operator=(Val && val) //
                -> CPP_ret(raw_storage_iterator &)(requires move_constructible<Val>)
            {
            ...
            CPP_member
            auto operator++(int) //
                -> CPP_ret(void)(requires(!forward_iterator<O>))
            {
                ++out_;
            }
            CPP_member
            auto operator++(int) //
                -> CPP_ret(raw_storage_iterator)(requires forward_iterator<O>)
            {
                auto tmp = *this;
                ++out_;
                return tmp;
