...
//
#ifndef EARANGES_ITERATOR_OPERATIONS_HPP
#define EARANGES_ITERATOR_OPERATIONS_HPP
...
namespace ranges
{
    ...
    struct advance_fn
    {
        ...
        //     i))]]
        {
            if constexpr(sized_sentinel_for<S, I>)
            {
                ...
                {
                    EARANGES_EXPECT(0 <= n && 0 <= d);
                    if(d <= n)
                    {
                        (*this)(i, eastl::move(bound));
                        return n - d;
                    }
                }
                (*this)(i, n);
    ...
    private:
        template<typename I>
        static constexpr void n_(I & i, iter_difference_t<I> n, eastl::input_iterator_tag);
        template<typename I>
        ...
        template<typename I, typename S>
        static constexpr void to_(I & i, S s, eastl::true_type); // assignable
        template<typename I, typename S>
        static constexpr void to_(I & i, S s, eastl::false_type); // !assignable
        template<typename I, typename S>
    ...
    /// \sa `advance_fn`
    EARANGES_INLINE_VARIABLE(advance_fn, advance)

#if EARANGES_CXX_IF_CONSTEXPR < EARANGES_CXX_IF_CONSTEXPR_17
    template<typename I>
    constexpr void advance_fn::n_(I & i, iter_difference_t<I> n, eastl::input_iterator_tag)
    {
        EARANGES_EXPECT(n >= 0);
        for(; n > 0; --n)
            ++i;
    ...
    // Advance to a certain position:
    template<typename I, typename S>
    constexpr void advance_fn::to_(I & i, S s, eastl::true_type)
    {
        i = static_cast<S &&>(s);
    }
    template<typename I, typename S>
    constexpr void advance_fn::to_(I & i, S s, eastl::false_type)
    {
    ...
    constexpr iter_difference_t<I> advance_fn::bounded_(I & it, iter_difference_t<I> n,
                                                        ...
                                                        Concept)
    {
        EARANGES_EXPECT(((bool)same_as<I, S> || 0 <= n));
        if(n == 0)
            return 0;
    ...
    /// \sa `next_fn`
    EARANGES_INLINE_VARIABLE(next_fn, next)
    ...
    /// \sa `prev_fn`
    EARANGES_INLINE_VARIABLE(prev_fn, prev)

    struct iter_enumerate_fn
    {
    private:
        template(typename I, typename S)(requires (!sized_sentinel_for<I, I>)) //
        static constexpr eastl::pair<iter_difference_t<I>, I> //
    ...
    public:
        template(typename I, typename S)(requires sentinel_for<S, I>)
        constexpr eastl::pair<iter_difference_t<I>, I> operator()(I first, S last) const
        {
            return iter_enumerate_fn::impl_i(static_cast<I &&>(first), static_cast<S &&>(last), sentinel_tag_of<S, I>());
        }
    };

    /// \sa `iter_enumerate_fn`
    EARANGES_INLINE_VARIABLE(iter_enumerate_fn, iter_enumerate)
    ...
    public:
        ...
        {
            return iter_distance_fn::impl_i(static_cast<I &&>(first),static_cast<S &&>(last), sentinel_tag_of<S, I>());
        }
    };

    /// \sa `iter_distance_fn`
    EARANGES_INLINE_VARIABLE(iter_distance_fn, iter_distance)
    ...
    private:
        ...
        template<typename I, typename S>
        static constexpr int impl_i(I first, S last, iter_difference_t<I> n, sized_sentinel_tag)
        {
    ...
    public:
        ...
        {
            return iter_distance_compare_fn::impl_i(static_cast<I &&>(first), static_cast<S &&>(last), n, sentinel_tag_of<S, I>());
        }
    };

    /// \sa `iter_distance_compare_fn`
    EARANGES_INLINE_VARIABLE(iter_distance_compare_fn, iter_distance_compare)

    // Like distance(b,e), but guaranteed to be O(1)
    struct iter_size_fn
    {
        template(typename I, typename S)(requires sized_sentinel_for<S, I>)
        constexpr meta::_t<eastl::make_unsigned<iter_difference_t<I>>> //
        operator()(I const & first, S last) const
        {
            using size_type = meta::_t<eastl::make_unsigned<iter_difference_t<I>>>;
    ...
    /// \sa `iter_size_fn`
    EARANGES_INLINE_VARIABLE(iter_size_fn, iter_size)
    ...
    namespace adl_uncounted_recounted_detail
    {
        ...
        struct recounted_fn
        {
            ...
            constexpr auto operator()(I i, J j, iter_difference_t<J> n) const -> decltype(recounted((I &&) i, (J &&) j, n))
            {
    ...
    EARANGES_INLINE_VARIABLE(adl_uncounted_recounted_detail::uncounted_fn, uncounted)
    EARANGES_INLINE_VARIABLE(adl_uncounted_recounted_detail::recounted_fn, recounted)

    struct enumerate_fn : iter_enumerate_fn
    {
    private:
        template<typename Rng>
        static constexpr eastl::pair<range_difference_t<Rng>, iterator_t<Rng>> impl_r(Rng & rng, range_tag, range_tag)
        {
            return iter_enumerate(begin(rng), end(rng));
        }
        template<typename Rng>
        static constexpr eastl::pair<range_difference_t<Rng>, iterator_t<Rng>> impl_r(Rng & rng, common_range_tag, sized_range_tag)
        {
    ...
    public:
        ...
        constexpr eastl::pair<range_difference_t<Rng>, iterator_t<Rng>> operator()(Rng && rng) const
        {
            // Better not be trying to compute the distance of an infinite range:
            EARANGES_EXPECT(!is_infinite<Rng>::value);
            return enumerate_fn::impl_r(rng, common_range_tag_of<Rng>(), sized_range_tag_of<Rng>());
        }
    };

    /// \sa `enumerate_fn`
    EARANGES_INLINE_VARIABLE(enumerate_fn, enumerate)
    ...
    /// \sa `distance_fn`
    EARANGES_INLINE_VARIABLE(distance_fn, distance)
    ...
    EARANGES_INLINE_VARIABLE(distance_compare_fn, distance_compare)
...
#endif // EARANGES_ITERATOR_OPERATIONS_HPP