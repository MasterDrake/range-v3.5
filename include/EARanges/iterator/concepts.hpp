...
#ifndef EARANGES_ITERATOR_CONCEPTS_HPP
#define EARANGES_ITERATOR_CONCEPTS_HPP
...
#include <EARanges/iterator/access.hpp>
...
/*TODO: I dont' think we should rely on std implementions...
        ...
        template(typename I)(requires same_as<I, class I::_Array_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
        template(typename I)(requires same_as<I, class I::_Array_const_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
        template(typename I)(requires same_as<I, class I::_Vector_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
        template(typename I)(requires same_as<I, class I::_Vector_const_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
        template(typename I)(requires same_as<I, class I::_String_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
        template(typename I)(requires same_as<I, class I::_String_const_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
        template(typename I)(requires same_as<I, class I::_String_view_iterator>)
        auto iter_concept_(I, priority_tag<3>) -> ranges::contiguous_iterator_tag;
...
*/
    ...
    namespace detail
    {
        ...
        template<typename D>
        inline constexpr bool _is_integer_like_ = eastl::is_integral<D>::value;
#else
        template<typename D, typename = void>
        constexpr bool _is_integer_like_ = eastl::is_integral<D>::value;
#endif
        ...
        concept (signed_integer_like_impl_)(D),
            integer_like_<D> AND
            concepts::type<eastl::integral_constant<bool, (D(-1) < D(0))>> AND
            eastl::integral_constant<bool, (D(-1) < D(0))>::value
        );
...
#endif // EARANGES_WORKAROUND_MSVC_792338
        // clang-format on
    } // namespace detail
      /// \endcond
    ...
    concept (sized_sentinel_for_)(S, I),
        (!disable_sized_sentinel<eastl::remove_cv_t<S>, eastl::remove_cv_t<I>>) AND
        sentinel_for<S, I>);
    ...
    CPP_concept input_iterator =
        ...
        CPP_concept_ref(ranges::with_category_, I, eastl::input_iterator_tag);
    ...
    CPP_concept forward_iterator =
        ...
        incrementable<I> &&
        sentinel_for<I, I> &&
        CPP_concept_ref(ranges::with_category_, I, eastl::forward_iterator_tag);
    ...
    CPP_concept bidirectional_iterator =
        ...
        CPP_concept_ref(ranges::with_category_, I, eastl::bidirectional_iterator_tag);
    ...
    CPP_concept random_access_iterator =
        ...
        CPP_concept_ref(ranges::with_category_, I, eastl::random_access_iterator_tag);

    /// \concept contiguous_iterator_
    /// \brief The \c contiguous_iterator_ concept
    template(typename I)(
    concept (contiguous_iterator_)(I),
        eastl::is_lvalue_reference<iter_reference_t<I>>::value AND
    ...
    namespace detail
    {
        ...
        template<typename T, typename U = meta::_t<eastl::remove_const<T>>>
...
#endif // EARANGES_ITERATOR_CONCEPTS_HPP
