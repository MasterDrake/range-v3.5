...
//
#ifndef EARANGES_ITERATOR_BASIC_ITERATOR_HPP
#define EARANGES_ITERATOR_BASIC_ITERATOR_HPP
...
#include <EARanges/detail/range_access.hpp>
...
namespace ranges
{
    ...
    struct basic_mixin : private box<T>
    {
        ...
        constexpr CPP_ctor(basic_mixin)()(                            //
            noexcept(eastl::is_nothrow_default_constructible<T>::value) //
        ...
        constexpr explicit CPP_ctor(basic_mixin)(T const & t)(     //
            noexcept(eastl::is_nothrow_copy_constructible<T>::value) //
    ...
    namespace detail
    {
        ...
        using cursor_reference_t =
            decltype(range_access::read(eastl::declval<Cur const &>()));
        ...
        using cursor_rvalue_reference_t =
            decltype(detail::cursor_move(eastl::declval<Cur const &>(), 42));
        ...
        {
            operator Head() const
                noexcept(noexcept(Head(Head(eastl::declval<Derived const &>().read_()))))
            {
        ...
        struct EARANGES_EMPTY_BASES basic_proxy_reference_
          : cursor_traits<Cur>
        ...
        public:
            constexpr reference_t_ read_() const noexcept(
                noexcept(reference_t_(range_access::read(eastl::declval<Cur const &>()))))
            {
        ...
        concept (cpp17_forward_cursor_)(C),
            eastl::is_reference<cursor_reference_t<C>>::value
        );
        ...
        template<typename Cur>
        using cursor_arrow_t = decltype(range_access::arrow(eastl::declval<Cur const &>()));
    ...
    public:
        ...
        {
            pos() = eastl::move(that.pos());
            return *this;
        }

        CPP_member
        constexpr auto operator*() const
            noexcept(noexcept(range_access::read(eastl::declval<Cur const &>())))
        ...
        constexpr auto operator*() //
            noexcept(noexcept(iter_reference_t{eastl::declval<Cur &>()})) //
        ...
        CPP_member
        constexpr auto operator*() const
            noexcept(noexcept(const_reference_t{eastl::declval<Cur const &>()}))
            -> CPP_ret(const_reference_t)(
                requires detail::has_cursor_next<Cur> &&
        ...
        constexpr detail::cursor_arrow_t<C> operator-> () const
            noexcept(noexcept(range_access::arrow(eastl::declval<C const &>())))
        {
        ...
        template(typename C = Cur)(
            requires (!detail::has_cursor_arrow<C>) AND detail::readable_cursor<C> AND
                eastl::is_lvalue_reference<const_reference_t>::value AND
                same_as<typename detail::iterator_associated_types_base<C>::value_type,
                        uncvref_t<const_reference_t>>)
        constexpr eastl::add_pointer_t<const_reference_t> operator-> () const
            noexcept(noexcept(*eastl::declval<basic_iterator const &>()))
        {
    ...
    private:
        constexpr basic_iterator post_increment_(eastl::false_type, int)
        {
        ...
        constexpr auto post_increment_(eastl::true_type, int) //
        ...
        constexpr void post_increment_(eastl::true_type, long)
        {
            ++*this;
        }
    ...
    public:
        ...
        template<typename C = Cur>
        friend constexpr auto iter_move(basic_iterator const & it) noexcept(noexcept(range_access::move(eastl::declval<C const &>())))
            -> CPP_broken_friend_ret(decltype(range_access::move(eastl::declval<C const &>())))(requires same_as<C, Cur> && detail::input_cursor<Cur>)
        {
    ...
    namespace _basic_iterator_
    {
        ...
        constexpr auto iter_move(basic_iterator<Cur> const & it) noexcept(
            noexcept(range_access::move(eastl::declval<Cur const &>())))
            -> CPP_broken_friend_ret(
                decltype(range_access::move(eastl::declval<Cur const &>())))(
    ...
    struct get_cursor_fn
    {
        ...
        constexpr Cur operator()(basic_iterator<Cur> && it) const
            noexcept(eastl::is_nothrow_move_constructible<Cur>::value)
        {
            return range_access::pos(eastl::move(it));
        }
    };

    /// \sa `get_cursor_fn`
    EARANGES_INLINE_VARIABLE(get_cursor_fn, get_cursor)
    /// @}
} // namespace ranges
...
//TODO:Capire perchè serve solo se c++20 :/
...
namespace ranges
{
    ...
    namespace detail
    {
        ...
        struct std_iterator_traits_
        {
            ...
            using iterator_concept = eastl::output_iterator_tag;
        };
