...
//
#ifndef EARANGES_ITERATOR_COMMON_ITERATOR_HPP
#define EARANGES_ITERATOR_COMMON_ITERATOR_HPP
    ...
    namespace ranges
    {
        ...
        private:
            ...
            template(typename J, typename R = iter_reference_t<J>)(
                requires eastl::is_reference<R>::value) //
                static meta::_t<eastl::add_pointer<R>> operator_arrow_(J const & j,
                                                                       long) noexcept
            {
                auto && r = *j;
            ...
            template(typename J, typename V = iter_value_t<J>)(
                requires constructible_from<V, iter_reference_t<J>>) static arrow_proxy_
                operator_arrow_(J const & j, ...) noexcept(noexcept(V(V(*j))))
            {
        ...
        public:
            ...
            common_iterator(I i)
              : data_(emplaced_index<0>, eastl::move(i))
            {}
            common_iterator(S s)
              : data_(emplaced_index<1>, eastl::move(s))
            {}
            ...
            template(typename I2,
                     typename S2)(requires convertible_to<I2, I> AND
                                      convertible_to<S2, S>) common_iterator &
            operator=(common_iterator<I2, S2> const & that)
            {
                detail::cidata(that).visit_i(emplace_fn{&data_});
                return *this;
            }
            iter_reference_t<I> operator*() //
                noexcept(noexcept(iter_reference_t<I>(*eastl::declval<I &>())))
            {
                return *ranges::get<0>(data_);
            }
            CPP_member
            auto operator*() const //
                noexcept(noexcept(iter_reference_t<I>(*eastl::declval<I const &>())))
                    -> CPP_ret(iter_reference_t<I>)(requires indirectly_readable<I const>)
            {
                return *ranges::get<0>(data_);
            }
            template(typename J = I)(requires indirectly_readable<J>) auto operator->()
                const //
                noexcept(noexcept(
                    common_iterator::operator_arrow_(eastl::declval<I const &>(), 42)))
                    -> decltype(common_iterator::operator_arrow_(
                        eastl::declval<J const &>(), 42))
            {
            ...
            CPP_member
            auto operator++(int) //
                -> CPP_ret(common_iterator)(requires forward_iterator<I>)
            {
            ...
            friend constexpr auto iter_move(common_iterator const & i) //
                noexcept(detail::has_nothrow_iter_move_v<I>) -> CPP_broken_friend_ret(
                    iter_rvalue_reference_t<I>)(requires input_iterator<I_>)
            {
        ...
        namespace _common_iterator_
        {
            ...
            auto iter_swap(common_iterator<I1, S1> const & x,
                ...
                noexcept(is_nothrow_indirectly_swappable<I1, I2>::value)
                    -> CPP_broken_friend_ret(void)(requires indirectly_swappable<I1, I2>)
            {
                return ranges::iter_swap(ranges::get<0>(detail::cidata(x)),
                                         ranges::get<0>(detail::cidata(y)));
            }
        ...
        template(typename I1, typename I2, typename S1,
                 typename S2)(requires sentinel_for<S1, I2> AND sentinel_for<S2, I1> AND(
            !equality_comparable_with<I1, I2>)) //
            bool
            operator==(common_iterator<I1, S1> const & x,
                       common_iterator<I2, S2> const & y)
        {
        ...
        template(typename I1, typename I2, typename S1,
                 typename S2)(requires sentinel_for<S1, I2> AND sentinel_for<S2, I1> AND
                                  equality_comparable_with<I1, I2>) bool
        operator==(common_iterator<I1, S1> const & x, common_iterator<I2, S2> const & y)
        {
            return detail::cidata(x).index() == 1u
                       ? (detail::cidata(y).index() == 1u ||
                          ranges::get<0>(detail::cidata(y)) ==
        ...
        template(typename I1, typename I2, typename S1,
                 typename S2)(requires sentinel_for<S1, I2> AND sentinel_for<S2, I1>) bool
        operator!=(common_iterator<I1, S1> const & x, common_iterator<I2, S2> const & y)
        {
            return !(x == y);
        }
        ...
        template(typename I1, typename I2, typename S1, typename S2)(
            requires sized_sentinel_for<I1, I2> AND sized_sentinel_for<S1, I2> AND
                sized_sentinel_for<S2, I1>) iter_difference_t<I2>
        operator-(common_iterator<I1, S1> const & x, common_iterator<I2, S2> const & y)
        {
            return detail::cidata(x).index() == 1u
                       ? (detail::cidata(y).index() == 1u
        ...
        struct indirectly_readable_traits<common_iterator<I, S>>
          : meta::if_c<(bool)indirectly_readable<I>, indirectly_readable_traits<I>,
                       meta::nil_>
        {};
        ...
        namespace detail
        {
            ...
            auto demote_common_iter_cat(long)
                -> with_iterator_category<eastl::input_iterator_tag>;
            template(typename I)(
                requires derived_from<
                    typename eastl::iterator_traits<I>::iterator_category,
                    eastl::forward_iterator_tag>) auto demote_common_iter_cat(int)
                -> with_iterator_category<eastl::forward_iterator_tag>;
              ...
              : decltype(detail::demote_common_iter_cat<I>(0))
            {
                ...
                using iterator_concept = meta::conditional_t<(bool)forward_iterator<I>,
...
#endif
                    explicit mixin(I it)
                      : mixin{cpp17_iterator_cursor{eastl::move(it)}}
                    {}
                    I base() const
                    {
                        return this->get().it_;
            ...
            public:
                ...
                CPP_member
                auto prev() //
                    -> CPP_ret(void)(requires bidirectional_iterator<I>)
                {
                    --it_;
                }
                CPP_member
                ...
                auto distance_to(cpp17_iterator_cursor const & that) //
                    -> CPP_ret(std::ptrdiff_t)(requires random_access_iterator<I>)
                {
                    auto d = that.it_ - it_;
                    EARANGES_EXPECT(d <= PTRDIFF_MAX);
...
EARANGES_DIAGNOSTIC_IGNORE_MISMATCHED_TAGS
    ...
    template<typename I, typename S>
    struct iterator_traits<::ranges::common_iterator<I, S>> : ::ranges::detail::common_iterator_std_traits<I>
    {};
} // namespace eastl
