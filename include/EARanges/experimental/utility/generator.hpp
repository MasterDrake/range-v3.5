...
//
#ifndef EARANGES_EXPERIMENTAL_UTILITY_GENERATOR_HPP
#define EARANGES_EXPERIMENTAL_UTILITY_GENERATOR_HPP
...
#include <cstddef>
#include <exception>
#include EARANGES_COROUTINES_HEADER
#include <utility>
...
#if defined(_MSC_VER) && !defined(EARANGES_SILENCE_COROUTINE_WARNING)
#ifdef __clang__
#pragma message(                                                 \
    "DANGER: clang doesn't (yet?) grok the MSVC coroutine ABI. " \
    "Use at your own risk. "                                     \
    "(EARANGES_SILENCE_COROUTINE_WARNING will silence this message.)")
#elif defined EARANGES_WORKAROUND_MSVC_835948
#pragma message(                                                                 \
    ...
    "(EARANGES_SILENCE_COROUTINE_WARNING will silence this message.)")
#endif
#endif // EARANGES_SILENCE_COROUTINE_WARNINGS
...
namespace ranges
{
    ...
    namespace experimental
    {
        ...
        struct EARANGES_EMPTY_BASES coroutine_owner;
    ...
    namespace detail
    {
        inline void resume(EARANGES_COROUTINES_NS::coroutine_handle<> coro)
        {
            // Pre: coro refers to a suspended coroutine.
            EARANGES_EXPECT(coro);
            EARANGES_EXPECT(!coro.done());
            coro.resume();
        }
    ...
    namespace experimental
    {
        ...
        template<typename Promise>
        struct EARANGES_EMPTY_BASES coroutine_owner
          : private EARANGES_COROUTINES_NS::coroutine_handle<Promise>
          , private detail::coroutine_owner_::adl_hook
        {
            CPP_assert(derived_from<Promise, enable_coroutine_owner>);
            using base_t = EARANGES_COROUTINES_NS::coroutine_handle<Promise>;
    ...
    namespace detail
    {
        ...
        struct generator_promise : experimental::enable_coroutine_owner
        {
            ...
            EARANGES_COROUTINES_NS::suspend_always initial_suspend() const noexcept
            {
                return {};
            }
            EARANGES_COROUTINES_NS::suspend_always final_suspend() const noexcept
            {
                return {};
            }
        ...
        struct sized_generator_promise : generator_promise<Reference>
        {
            ...
            EARANGES_COROUTINES_NS::suspend_never initial_suspend() const noexcept
            {
                // sized_generator doesn't suspend at its initial suspend point because...
                return {};
            }
            EARANGES_COROUTINES_NS::suspend_always await_transform(
            ...
            {
                EARANGES_EXPECT(size_ != experimental::generator_size::invalid);
    ...
    namespace experimental
    {
        ...
        struct generator : view_facade<generator<Reference, Value>>
        {
            ...
            {
                EARANGES_EXPECT(coro_);
            }

        private:
            friend range_access;
            friend struct sized_generator<Reference, Value>;
            using handle = EARANGES_COROUTINES_NS::coroutine_handle<promise_type>;
...
#endif // EARANGES_CXX_COROUTINES >= EARANGES_CXX_COROUTINES_TS1

#endif // EARANGES_EXPERIMENTAL_UTILITY_GENERATOR_HPP
