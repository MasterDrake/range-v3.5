...
void forward_test()
{
    ...
    // This assertation fails because unlike std::forward_list, eastl::slist implements size(), even if when not caching mSize. Therefore the concept is still valid despite the requirements of amortized constant-time complexity are not met.
