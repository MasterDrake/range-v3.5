...
constexpr bool test_constexpr()
{
    ...
    int r1[10] = {0};
    int r2[10] = {0};
    typedef eastl::tuple<int const *, int *, int *> P;
    P p = partition_move(ia, r1, r2, is_odd());
    STATIC_CHECK_RETURN(eastl::get<0>(p) == eastl::end(ia));
    STATIC_CHECK_RETURN(eastl::get<1>(p) == r1 + 4);
    STATIC_CHECK_RETURN(r1[0] == 1);
    STATIC_CHECK_RETURN(r1[1] == 3);
    STATIC_CHECK_RETURN(r1[2] == 5);
    STATIC_CHECK_RETURN(r1[3] == 7);
    STATIC_CHECK_RETURN(eastl::get<2>(p) == r2 + 4);
