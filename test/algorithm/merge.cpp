...
int main()
{
    {
        int N = 100000;
        eastl::unique_ptr<int[]> ia{new int[N]};
        eastl::unique_ptr<int[]> ib{new int[N]};
        eastl::unique_ptr<int[]> ic{new int[2 * N]};
        for(int i = 0; i < N; ++i)
            ia[i] = 2 * i;
        for(int i = 0; i < N; ++i)
            ib[i] = 2 * i + 1;
...

    {
        ...
        auto r = ranges::merge(eastl::move(r0), eastl::move(r1), ic.get());
