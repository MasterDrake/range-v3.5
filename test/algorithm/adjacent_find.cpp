...
int main()
{
    ...
    eastl::pair<int, int> v2[] = {{0, 0}, {0, 2}, {0, 2}, {0, 4}, {0, 6}};
    CHECK(ranges::adjacent_find(ranges::begin(v2), ranges::end(v2), ranges::equal_to{}, &eastl::pair<int, int>::second) == &v2[1]);
    CHECK(ranges::adjacent_find(v2, ranges::equal_to{}, &eastl::pair<int, int>::second) == &v2[1]);
    static_assert(eastl::is_same<eastl::pair<int,int>*,
                               decltype(ranges::adjacent_find(v2, ranges::equal_to{},
                                    &eastl::pair<int, int>::second))>::value, "");

    {
        ...
        constexpr eastl::pair<int, int> a2[] = {{0, 0}, {0, 2}, {0, 2}, {0, 4}, {0, 6}};
